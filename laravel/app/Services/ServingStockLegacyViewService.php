...
final class ServingStockLegacyViewService
{
    ...
    public function data(Request $request, object $stock): array
    {
        ...
        $fallbackHorizon = $horizons->firstWhere('days', 20)
            ?? $horizons->firstWhere('active_prediction_enabled', true)
            ?? $horizons->first();
        $fallbackModel = $fallbackHorizon
            ? ($fallbackHorizon['variants'][$fallbackHorizon['active_variant']] ?? null)
            : null;
        $signal = strtoupper((string) ($currentSignal?->signal
            ?? $overview['release']['recommended_signal']
            ?? data_get($fallbackModel, 'prediction.signal')
            ?? 'HOLD'));
        if ($signal === 'NEUTRAL') {
            $signal = 'HOLD';
        }
        // The model-statistics panel must reflect the horizon+variant that
        // actually produced the displayed signal (from buy_scopes/watch_context/
        ...
        $primaryModel = $primaryHorizon
            ? ($primaryHorizon['variants'][$primaryVariant] ?? $primaryHorizon['variants'][$primaryHorizon['active_variant']] ?? null)
            : null;
        ...
        // Generated by App\Console\Commands\GenerateInstrumentDescriptions,
        // stored on the main instruments table and mirrored into
        // serving_instruments.display_metadata for this read side - same
        // locale-fallback order the screener already uses for these fields.
        $isEnglish = app()->getLocale() === 'en';
        $businessSummary = $isEnglish
            ? (data_get($stock->display_metadata, 'business_summary_en') ?: data_get($stock->display_metadata, 'business_summary'))
            : (data_get($stock->display_metadata, 'business_summary') ?: data_get($stock->display_metadata, 'business_summary_en'));
        $businessDescription = $isEnglish
            ? (data_get($stock->display_metadata, 'business_description_en') ?: data_get($stock->display_metadata, 'business_description'))
            : (data_get($stock->display_metadata, 'business_description') ?: data_get($stock->display_metadata, 'business_description_en'));
        ...
        // The identical number the screener shows for this instrument
        // (ServingScreenerService::compositeScores(), same cached computation
        ...
        if ($compositeScore === null) {
            ...
            // untradeable edge case) - fall back to a local approximation
        ...
        // Written opportunities/risks/key-factors assessment (generated by
        // App\Console\Commands\GenerateStockAiAssessments, one per instrument
        // per day) - main app DB, keyed by the same instrument_id serving
        // rows already carry.
        $aiAssessment = DB::table('stock_ai_assessments')
            ->where('instrument_id', $stock->instrument_id)
        ...
        return [
            ...
            'aiAssessmentOpportunities' => $aiAssessmentOpportunities,
            'aiAssessmentRisks' => $aiAssessmentRisks,
            'aiAssessmentFactors' => $aiAssessmentFactors,
