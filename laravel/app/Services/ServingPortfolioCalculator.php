...
final class ServingPortfolioCalculator
{
    ...
    public const ALLOCATION_DYNAMIC_WEIGHT = 'dynamic_weight';
    ...
    public function calculate(
        ...
        if ($initialCapital <= 0 || $maximumPositions < 1
            || ! in_array($allocationMode, [self::ALLOCATION_FULL_INVESTMENT, self::ALLOCATION_EQUAL_WEIGHT, self::ALLOCATION_DYNAMIC_WEIGHT], true)
        ...
        $closePositions = function (string $date, bool $sameDay) use (
            ...
            $closingKeys = array_keys(array_filter(
                $positions,
                fn (array $position): bool => $sameDay
        ...
        foreach (array_keys($timeline) as $date) {
            // A daily-close position remains invested through its exit date.
            // Capital and the instrument slot become reusable on the next day.
            $closePositions($date, false);
            ...
            foreach ($candidates as $candidate) {
                ...
                $availableBudget = match ($allocationMode) {
                    self::ALLOCATION_EQUAL_WEIGHT => min($cash, $equalWeightBudget),
                    self::ALLOCATION_DYNAMIC_WEIGHT => min($cash, $equityBeforeEntries * $maxStockAllocationRate),
                    default => $cash,
                };
                ...
                $maximumStockNotional = in_array($allocationMode, [self::ALLOCATION_EQUAL_WEIGHT, self::ALLOCATION_DYNAMIC_WEIGHT], true)
                    ? $equityBeforeEntries * $maxStockAllocationRate
                    : $maximumAffordableNotional;
                $notionalLimit = min($maximumAffordableNotional, $maximumStockNotional);
                $quantity = (int) floor(($notionalLimit + 0.000001) / (float) $candidate['entry_price']);
                if ($quantity < 1) {
                    $skippedCash++;

                    continue;
                }
                $positionNotional = $quantity * (float) $candidate['entry_price'];
                ...
                $positions[$positionKey] = [
                    ...
                    'effective_max_stock_allocation_percent' => in_array($allocationMode, [self::ALLOCATION_EQUAL_WEIGHT, self::ALLOCATION_DYNAMIC_WEIGHT], true)
                        ? $maxStockAllocationRate * 100
                        : 100.0,
                ];
            }

            $closePositions($date, true);
        ...
        $totalDeployedCapital = array_sum($positionNotionals);
        $modelReferences = array_map(function (array $reference) use ($completed, $totalDeployedCapital): array {
            $modelTrades = array_values(array_filter(
                $completed,
                fn (array $trade): bool => (string) $trade['configuration_key'] === (string) $reference['configuration_key'],
            ));
            $deployedCapital = array_sum(array_map(
                fn (array $trade): float => (float) $trade['position_notional'],
                $modelTrades,
            ));
            $costs = array_sum(array_map(
                fn (array $trade): float => (float) $trade['buy_fee'] + (float) $trade['sell_fee'],
                $modelTrades,
            ));
            $netProfit = array_sum(array_map(
                fn (array $trade): float => (float) $trade['profit'],
                $modelTrades,
            ));

            return [
                ...$reference,
                'capital_rate_percent' => $totalDeployedCapital > 0
                    ? round($deployedCapital / $totalDeployedCapital * 100, 2)
                    : 0.0,
                'simulation_costs' => round($costs, 2),
                'simulation_net_profit' => round($netProfit, 2),
            ];
        }, $modelReferences);
        $finalCapital = $cash;
        ...
        return [
            ...
            'calculation_version' => 'serving-portfolio-whole-shares-v2',
            ...
            'maximum_capital_utilization_percent' => $utilizationSamples !== []
                ...
                : 0.0,
            'allocation_mode' => $allocationMode,
            'allocation_mode_label' => match ($allocationMode) {
                self::ALLOCATION_FULL_INVESTMENT => 'Voll investieren',
                self::ALLOCATION_DYNAMIC_WEIGHT => 'Flexible Gewichtung',
                default => 'Gleichgewichtet',
            },
            'initial_equal_weight_budget' => in_array($allocationMode, [self::ALLOCATION_EQUAL_WEIGHT, self::ALLOCATION_DYNAMIC_WEIGHT], true)
