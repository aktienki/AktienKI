...
final class EToroBroker
{
    ...
    public function place(BrokerConnection $connection, array $order): array
    {
        ...
        if (! $instrumentId) {
            throw new RuntimeException('eToro-Instrument konnte nicht aufgelöst werden.');
        }
        ...
        if ($order['stop_loss'] ?? null) {
            $payload['stopLossRate'] = (float) $order['stop_loss'];
        }
        if ($order['take_profit'] ?? null) {
            $payload['takeProfitRate'] = (float) $order['take_profit'];
        }
