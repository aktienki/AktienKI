    ...
    public function handle(): int
    {
        ...
        foreach ((array) ($report['walk_forward'] ?? []) as $fold) {
            foreach ((array) data_get($fold, 'metrics._point_in_time', []) as $point) {
                ...
                if ($sector === '' || $date === '' || ! is_numeric($point['probability_60d'] ?? null)) {
                    continue;
                }
        ...
        $now = now();
        $sectorRows = [];
        foreach ($byDate as $date => $sectors) {
            foreach ($sectors as $sector => $probability) {
                $sectorRows[] = [
        ...
        foreach (array_chunk($sectorRows, 500) as $chunk) {
        ...
        $indexRows = [];
        foreach ($byDate as $date => $sectors) {
            foreach ($indexSectorWeights as $indexId => $weights) {
                $sum = 0.0;
                $covered = 0;
                foreach ($weights as $sector => $weight) {
                    if (isset($sectors[$sector])) {
                        $sum += $sectors[$sector] * $weight;
                        $covered += $weight;
                    }
                }
                if ($covered === 0) {
        ...
        foreach (array_chunk($indexRows, 500) as $chunk) {
            DB::table('market_context_predictions')->upsert(
                $chunk, ['prediction_date', 'scope_type', 'scope_key'], ['score', 'confidence', 'signal', 'member_count', 'meta', 'updated_at']
            );
        }
