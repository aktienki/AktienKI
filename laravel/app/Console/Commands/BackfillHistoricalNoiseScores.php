...
final class BackfillHistoricalNoiseScores extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (in_array(0, $runs, true)) {
            $this->error('Alle vier Run-IDs sind erforderlich.');
        ...
        foreach ($dates as $date) {
            ...
            foreach ([[0, 5], [5, 10], [10, 15], [15, 20]] as [$from,$to]) {
                [$pos,$neg] = $this->segment($points[$from], $points[$to], $to - $from);
                $positive += $pos;
                $negative += $neg;
            }
        ...
        foreach (array_chunk($rows, 500) as $chunk) {
            DB::table('historical_noise_scores')->upsert(
        ...
        $this->info(count($rows).' Noise-Scores gespeichert.');
    ...
    {
        if ($start >= 0 && $end >= 0) {
            return [($start + $end) * $width / 2, 0.0];
        }
        if ($start <= 0 && $end <= 0) {
            return [0.0, ($start + $end) * $width / 2];
        }
