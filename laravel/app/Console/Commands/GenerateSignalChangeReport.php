...
class GenerateSignalChangeReport extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (! $transition) {
            $this->error('Kein Signalwechsel in den letzten 30 Tagen gefunden.');
    ...
    private function findTransition(string $symbol): ?object
    {
        ...
        if ($symbol !== '') {
            $query->whereRaw('UPPER(i.symbol) = ?', [strtoupper($symbol)]);
        }
    ...
    private function generateText(array $data, string $locale): array
    {
        ...
        if ($key === '') {
            return [$locale === 'en' ? 'Offline report without AI text. OPENAI_API_KEY is not configured.' : 'Offline-Bericht ohne KI-Text. OPENAI_API_KEY ist nicht konfiguriert.', []];
        }
        $prompt = ($locale === 'en'
    ...
    private function chartSvg(array $bars, float $current, float $target): string
    {
        ...
        if (count($bars) < 2) {
            return '<svg xmlns="http://www.w3.org/2000/svg" width="720" height="220"><text x="20" y="40">'.($english ? 'Insufficient price data' : 'Keine ausreichenden Kursdaten').'</text></svg>';
        }
        $values = array_column($bars, 'c');
        $min = min($values);
        $max = max($values);
        ...
        foreach ($values as $i => $v) {
            $points[] = round(20 + $i * ($w - 40) / (count($values) - 1), 1).','.round($h - 20 - ($v - $min) / $range * ($h - 40), 1);
        }
