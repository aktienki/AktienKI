...
class ApplyHorizonFusion extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($rows as $instrumentRows) {
            ...
            foreach (self::HORIZONS as $minutes => $days) {
                $column = "market_return_{$days}d";
                $row = $latest->get($minutes);
                if (! $row || ! is_numeric($row->{$column} ?? null)) {
                    $returns = [];
                    break;
                }
            ...
            sort($sorted);
            $median = ($sorted[1] + $sorted[2]) / 2.0;
            $dev = array_map(fn ($v) => abs($v - $median), $sorted);
            sort($dev);
            $mad = ($dev[1] + $dev[2]) / 2.0;
            ...
            foreach ([[0, 5], [5, 10], [10, 15], [15, 20]] as [$from, $to]) {
                ...
                $positiveArea += $segmentPositive;
                $negativeArea += $segmentNegative;
            ...
            for ($index = 1; $index < count($segmentSlopes); $index++) {
                if ($segmentSlopes[$index - 1] * $segmentSlopes[$index] < 0) {
                    $directionReversals++;
                }
            }
            $zeroCrossings = 0;
            ...
            for ($index = 1; $index < count($pointValues); $index++) {
                if ($pointValues[$index - 1] * $pointValues[$index] < 0) {
                    $zeroCrossings++;
                }
            }
            $details = [
    ...
    {
        if ($start >= 0 && $end >= 0) {
            return [($start + $end) * $width / 2.0, 0.0];
        }
        if ($start <= 0 && $end <= 0) {
            return [0.0, ($start + $end) * $width / 2.0];
        }
        $crossing = $width * abs($start) / (abs($start) + abs($end));
        if ($start > 0) {
            return [$start * $crossing / 2.0, $end * ($width - $crossing) / 2.0];
        }
