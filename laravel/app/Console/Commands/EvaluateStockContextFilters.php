...
final class EvaluateStockContextFilters extends Command
{
    ...
    public function handle(HistoricalActionScoreService $scoreService): int
    {
        ...
        if (! $instrument) {
            $this->error('Aktie nicht gefunden.');
        ...
        if (! $threshold || ! is_numeric($threshold->minimum_ai_score)) {
            $this->error('Individuelle Rohschwelle fehlt.');
        ...
        if (! $runId || ! $split) {
            $this->error('Kalibrierungslauf oder OOS-Split fehlt.');
        ...
        if ($this->option('index')) {
            $indexQuery->where('market_index.symbol', (string) $this->option('index'));
        }
        ...
        for ($mask = 1; $mask < (1 << count($filterNames)); $mask++) {
            ...
            if ($candidate['calibration']['trades'] < $minimumEvents || $candidate['oos']['trades'] < $minimumEvents) {
                continue;
            }
            if (! $this->noHarm($candidate['calibration'], $baseline['calibration'])) {
                continue;
            }
            $candidates->push($candidate);
        }
        $ranked = $candidates->sortByDesc(fn (array $candidate): float => (float) ($candidate['calibration']['hit_rate'] ?? 0)
    ...
    private function evaluate(Collection $rows, float $cutoff, array $selectedFilters, array $filters, string $split): array
    {
        ...
        $entries = $rows->filter(function (object $row) use ($cutoff, $selectedFilters, $filters, &$previous): bool {
            ...
            foreach ($selectedFilters as $filter) {
                $accepted = $accepted && $filters[$filter]($row);
            }
    ...
    private function noHarm(array $candidate, array $baseline): bool
    {
        foreach (['hit_rate', 'profit_factor', 'average_return_percent'] as $metric) {
            if (! is_numeric($candidate[$metric] ?? null) || ! is_numeric($baseline[$metric] ?? null)) {
                return false;
            }
            if ((float) $candidate[$metric] + 1e-9 < (float) $baseline[$metric]) {
                return false;
            }
        }
        if (! is_numeric($candidate['maximum_drawdown_percent'] ?? null)
            || ! is_numeric($baseline['maximum_drawdown_percent'] ?? null)
            || (float) $candidate['maximum_drawdown_percent'] > (float) $baseline['maximum_drawdown_percent'] + 1e-9) {
            return false;
        }
    ...
    {
        $n = (int) ($stats['trades'] ?? 0);
        $hit = (float) ($stats['hit_rate'] ?? 0);
        $pf = (float) ($stats['profit_factor'] ?? 0);
        $avg = (float) ($stats['average_return_percent'] ?? 0);
        if ($n >= 20 && $hit >= 65 && $pf >= 1.5 && $avg > 0) {
            return 'quality';
        }
        if ($n >= 10 && $hit >= 60 && $pf >= 1.5 && $avg > 0) {
            return 'solid';
        }
        if ($n >= 1 && $hit >= 55 && $pf >= 1.2 && $avg > 0) {
            return 'basic';
        }
