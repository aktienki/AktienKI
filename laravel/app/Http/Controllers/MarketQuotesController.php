            ...
            return $bars->map(function ($symbolBars): array {
                ...
                $previous = $symbolBars
                    ->first(fn (object $bar): bool => $bar->interval === '1d'
                        && Carbon::parse($bar->bar_time)->lt(
                            Carbon::parse($latest->bar_time)->startOfDay()
                        ));
                $price = (float) $latest->close;
                if ($previous && (! is_numeric($previous->close) || (float) $previous->close < $price * .5 || (float) $previous->close > $price * 2)) {
                    $previous = $symbolBars->first(fn (object $bar): bool => $bar->interval === '1d'
                        && is_numeric($bar->close)
                        && Carbon::parse($bar->bar_time)->lt(Carbon::parse($latest->bar_time)->startOfDay())
