<x-app-layout>
    ...
    <style>
        ...
        @media (max-width:767px) {
            ...
            @keyframes screener-dynamic-marker-pulse{0%,100%{filter:brightness(1);opacity:.82}50%{filter:brightness(1.35);opacity:1}}
                    ...
                    <a href="{{ route('stocks.show', array_filter(['symbol' => $stock->symbol, 'prediction' => ($stock->data_source ?? null) === 'serving' ? null : $stock->id, 'return_to' => request()->getRequestUri()], fn ($value) => $value !== null)) }}" class="screener-desktop-summary hidden w-full items-center gap-4 text-left md:grid" aria-label="{{ __('Details zu :stock anzeigen', ['stock' => $stock->name ?: $stock->symbol]) }}">
                        ...
                        <span class="screener-desktop-forecasts">
                            ...
                            @foreach($mobileForecasts as $days => $forecast)
                                <i data-assessment-horizon="{{ $days }}" data-assessment-return="{{ is_numeric($forecast) ? number_format((float) $forecast, 6, '.', '') : '' }}" data-assessment-base-return="{{ is_numeric($forecast) ? number_format((float) $forecast, 6, '.', '') : '' }}" class="{{ $triggerHorizon === (int) $days ? 'is-trigger-horizon '.(($realtimeQuotes ?? false) && $showDynamicRiskBorder ? 'has-dynamic-risk '.((float) $dynamicRiskPercent >= 90 ? 'is-critical-dynamic-risk ' : '') : '') : '' }}{{ is_numeric($forecast) && (float) $forecast < 2.0 ? 'is-time-downgraded ' : '' }}{{ is_numeric($forecast) && (float) $forecast < 0 ? 'is-negative-forecast' : '' }}" title="{{ __('Nettoprognose nach :cost % erwarteten Kosten', ['cost' => number_format($forecastCostPercent, 2, ',', '.')]).($triggerHorizon === (int) $days ? ' · '.__('Signalauslösender Horizont').' · '.$days.'T' : '') }}"><small>{{ $days }}T</small><strong
            ...
            (() => {
                ...
                const updateForecastWarnings=(row)=>{
                    ...
                    row.querySelectorAll('.is-trigger-horizon').forEach(cell=>{
                        const hasDynamicRisk=netForecast<2&&risk>baseRisk+.01;
