<x-app-layout>
    ...
    <style>
        ...
        @media (min-width:768px) {
            .screener-page .screener-table-head{grid-template-columns:42px minmax(330px,1fr) 78px 190px 64px 124px 124px repeat(3,72px)!important}
            .screener-page .screener-desktop-summary{grid-template-columns:minmax(330px,1fr) 78px 190px 64px 124px 124px repeat(3,72px)!important}
            ...
            .screener-page .screener-desktop-panel{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.12rem;box-sizing:border-box;width:64px!important;min-width:64px!important;max-width:64px!important}
            .screener-page .screener-desktop-panel>small{color:var(--ak-muted);font-size:.5rem;font-weight:800;letter-spacing:.03em;text-transform:uppercase}
            .screener-page .screener-desktop-panel>strong{font-size:.74rem;font-weight:900;line-height:1;font-variant-numeric:tabular-nums}
            .screener-page .screener-desktop-panel>i{color:var(--ak-muted);font-size:.5rem;font-weight:700;font-style:normal;font-variant-numeric:tabular-nums}
            .screener-page .screener-desktop-scale-grade{display:grid!important;width:124px!important;min-width:124px!important;align-content:center;align-items:stretch!important;justify-self:stretch;gap:.48rem;padding:0 .25rem}
            ...
            .screener-page .screener-desktop-scale-grade>.screener-desktop-scale{position:relative;display:block!important;box-sizing:border-box;width:114px!important;min-width:114px!important;max-width:114px!important;height:.34rem;align-self:center;flex:0 0 114px!important;border-radius:.12rem;font-style:normal}
            ...
            .screener-page .screener-table-head>span:not(:first-child),.screener-page .screener-desktop-summary>.screener-desktop-price,.screener-page .screener-desktop-summary>.screener-desktop-panel,.screener-page .screener-desktop-summary>.screener-desktop-signal,.screener-page .screener-desktop-summary>.screener-desktop-grade,.screener-page .screener-desktop-summary>.screener-desktop-forecasts>i,.screener-page .screener-desktop-summary>svg{border-left:1px solid color-mix(in srgb,var(--ak-muted) 13%,transparent)!important}
            ...
            .screener-page .screener-desktop-summary>.screener-desktop-forecasts>i{border:1px solid color-mix(in srgb,var(--ak-muted) 16%,transparent)!important;border-radius:.48rem!important;background:transparent!important}
            .screener-page .screener-desktop-forecasts>i.is-trigger-horizon{border-left-color:color-mix(in srgb,var(--ak-muted) 13%,transparent)!important;border-top:2px solid color-mix(in srgb,#22d3ee 72%,transparent)!important;background:transparent!important;box-shadow:none!important}
            .screener-page .screener-desktop-forecasts>i.has-improved-forecast{border-bottom:2px solid #34d399!important}
            ...
            .screener-page .screener-desktop-forecasts>i.has-dynamic-risk{border-bottom:2px solid #fbbf24!important}
            .screener-page .screener-desktop-forecasts>i.has-dynamic-risk.is-critical-dynamic-risk{border-bottom-color:#fb7185!important}
            ...
            .screener-page .screener-desktop-row{display:grid;grid-template-columns:42px minmax(0,1fr);align-items:stretch}
            .screener-page .screener-desktop-watchlist-column{position:relative;z-index:35;display:grid!important;width:42px;height:100%;min-height:100%;align-self:stretch;justify-self:center;place-items:center;margin:0;padding:0;border-right:1px solid color-mix(in srgb,var(--ak-muted) 18%,transparent)}
            .screener-page .screener-desktop-watchlist-column>summary{align-self:center;justify-self:center;margin:auto}
            .screener-page .screener-desktop-watchlist-column>summary::-webkit-details-marker{display:none}
            .screener-page .screener-watchlist-head{display:grid;width:42px;height:100%;place-items:center;margin:0;padding:0;color:#f59e0b;text-align:center}
            .screener-page .screener-watchlist-head svg{width:1rem;height:1rem}
            ...
            .screener-page .screener-desktop-scale>.screener-dynamic-risk-marker{position:absolute;top:50%;left:clamp(2%,var(--dynamic-risk-position),98%);z-index:4;width:2px;height:1rem;border-radius:999px;background:var(--dynamic-risk-color,#fbbf24);transform:translate(-50%,-50%)}
        }
        @media (max-width:767px) {
            ...
            .screener-page .sms-v2-forecast>i.is-trigger-horizon{border-top:2px solid color-mix(in srgb,#22d3ee 72%,transparent)!important}
            .screener-page .sms-v2-forecast>i.has-improved-forecast{border-bottom:2px solid #34d399!important}
            .screener-page .sms-v2-forecast>i.is-time-downgraded{border-bottom:2px solid #fbbf24!important}
            .screener-page .sms-v2-forecast>i.is-time-downgraded>b:not(.text-rose-400){color:#fbbf24!important}
    ...
    <div data-simulate-live="{{ $simulateLiveQuotes ? '1' : '0' }}" x-data="{ filtering: false, submitFilters(form) { this.filtering = true; requestAnimationFrame(() => form.submit()) } }" @pageshow.window="filtering = false" class="screener-page mx-auto max-w-[96rem] px-3 py-5 text-[var(--ak-text)] sm:px-5 lg:py-8">
        ...
        <form x-cloak x-show.important="filtersOpen" method="GET" action="{{ route('screener.index') }}" class="screener-filter-bar mt-2 flex flex-wrap gap-2 rounded-lg border border-cyan-400/30 bg-[var(--ak-card)] p-3 text-[17px] shadow-[var(--ak-shadow)]" style="font-size:17px">
            ...
            <label class="ak-input flex h-10 shrink-0 cursor-pointer items-center gap-2 whitespace-nowrap px-3 text-xs font-bold" style="flex:1 1 260px" title="{{ __('Nur Aktien zeigen, deren Modell den eigenen Drei-Jahres-Qualitäts-Gate bestanden hat.') }}">
        ...
        <div class="screener-results-scroll transition-opacity duration-200" :class="filtering ? 'opacity-45' : 'opacity-100'">
        <section class="screener-desktop-table grid grid-cols-1 gap-4">
            ...
            @forelse($stocks as $stock)
                @php
                    ...
                    $baseRankingRiskPercent = $rankingRiskPercent;
                ...
                @php
                    ...
                    $activeForecastHorizon = $triggerHorizon !== null && $mobileForecasts->has($triggerHorizon)
                        ? $triggerHorizon
                        : $primaryForecastHorizon;
                    $signalStartedAt = filled($stock->signal_transition_at ?? null)
                        ? \Illuminate\Support\Carbon::parse($stock->signal_transition_at)->startOfDay()
                        : (filled($stock->prediction_time ?? null) ? \Illuminate\Support\Carbon::parse($stock->prediction_time)->startOfDay() : null);
                    $elapsedSignalTradingDays = $signalStartedAt && $signalStartedAt->isPast()
                        ? (int) floor($signalStartedAt->diffInWeekdays(now()))
                        : 0;
                    $signalRemainingTradingDays = max(0, $activeForecastHorizon - $elapsedSignalTradingDays);
                    $signalValidityLabel = $activeForecastHorizon.'T · '.__('noch').' '.$signalRemainingTradingDays.'T';
                    $signalValidityTitle = __('Signalauslösender Horizont').': '.$activeForecastHorizon.' '.__('Handelstage')
                        .' · '.__('Voraussichtlich noch gültig').': '.$signalRemainingTradingDays.' '.__('Handelstage')
                        .($signalStartedAt ? ' · '.__('Signal seit').': '.$signalStartedAt->format('d.m.Y') : '');
                    $activeNetForecast = $mobileForecasts->get($activeForecastHorizon);
                    ...
                    $showDynamicRiskBorder = $showDynamicRisk
                        ...
                        && floor((float) $dynamicRiskPercent / 10) > floor((float) $baseRankingRiskPercent / 10);
                    $dynamicRiskColor = $showDynamicRisk
                        ? ((float) ($dynamicRiskPercent ?? 0) >= 90 ? '#fb7185' : '#fbbf24')
                        : '#22d3ee';
                @endphp
                <article
                    ...
                    <a href="{{ route('stocks.show', array_filter(['symbol' => $stock->symbol, 'prediction' => ($stock->data_source ?? null) === 'serving' ? null : $stock->id, 'return_to' => request()->getRequestUri()], fn ($value) => $value !== null)) }}" class="screener-desktop-summary w-full items-center gap-4 text-left md:grid" aria-label="{{ __('Details zu :stock anzeigen', ['stock' => $stock->name ?: $stock->symbol]) }}">
                        ...
                        <span class="screener-desktop-price">
                            <span><strong
                                ...
                                @endif
                            >{{ is_numeric($stock->current_price) ? number_format((float) $stock->current_price, 2, ',', '.') : '—' }} <em>{{ $displayCurrencySymbol }}</em>@if($realtimeQuotes ?? false)<i class="screener-live-dot" data-screener-live-status="{{ $stock->symbol }}" title="{{ __('Nicht live') }}"></i>@endif</strong>@if($priceChange !== null)<b class="{{ $priceChange >= 0 ? 'text-emerald-400' : 'text-rose-400' }}">{{ ($priceChange > 0 ? '+' : '').number_format($priceChange, 2, ',', '.').' %' }}</b>@endif</span>
                        ...
                        <span class="screener-desktop-grade screener-desktop-scale-grade">
                            ...
                            <i class="screener-desktop-scale risk" style="--position:{{ number_format(100 - ($rankingRiskPercent ?? 0), 2, '.', '') }}%;--marker:{{ $riskDonutColor }}" role="meter" aria-label="{{ __('Risiko') }} {{ \App\Support\QualityGrade::risk($rankingRiskPercent) ?? '—' }}" aria-valuemin="0" aria-valuemax="100" @if($rankingRiskPercent !== null) aria-valuenow="{{ number_format(100 - $rankingRiskPercent, 1, '.', '') }}" @endif><em @if($rankingRiskPercent === null) hidden @endif></em>@if(($realtimeQuotes ?? false) && is_numeric($baseRankingRiskPercent))<b class="screener-dynamic-risk-marker" data-screener-dynamic-risk style="--dynamic-risk-position:{{ number_format(100 - (float) ($dynamicRiskPercent ?? 0), 2, '.', '') }}%;--dynamic-risk-color:{{ $dynamicRiskColor }}" title="{{ __('Dynamisches Risiko') }}: {{ number_format((float) ($dynamicRiskPercent ?? 0), 1, ',', '.') }} %" @if(!is_numeric($activeNetForecast) || (float) $activeNetForecast >= 2) hidden @endif></b>@endif</i>
                        </span>
                        <span class="screener-desktop-forecasts">
                            ...
                            @foreach($mobileForecasts as $days => $forecast)
                                <i data-assessment-horizon="{{ $days }}" data-assessment-return="{{ is_numeric($forecast) ? number_format((float) $forecast, 6, '.', '') : '' }}" data-assessment-base-return="{{ is_numeric($forecast) ? number_format((float) $forecast, 6, '.', '') : '' }}" class="{{ $triggerHorizon === (int) $days ? 'is-trigger-horizon '.(($realtimeQuotes ?? false) && $showDynamicRiskBorder ? 'has-dynamic-risk '.((float) $dynamicRiskPercent >= 90 ? 'is-critical-dynamic-risk ' : '') : '') : '' }}{{ is_numeric($forecast) && (float) $forecast < 2.0 && ($triggerHorizon !== (int) $days || $showDynamicRiskBorder) ? 'is-time-downgraded ' : '' }}{{ is_numeric($forecast) && (float) $forecast < 0 ? 'is-negative-forecast' : '' }}" title="{{ __('Nettoprognose nach :cost % erwarteten Kosten', ['cost' => number_format($forecastCostPercent, 2, ',', '.')]).($triggerHorizon === (int) $days ? ' · '.__('Signalauslösender Horizont').' · '.$days.'T' : '') }}"><small>{{ $days }}T</small><strong
    ...
    @once
        <script>
            (() => {
                ...
                const updateForecastWarnings=(row)=>{
                    if(!row)return;
                    ...
                    const baseForecast=Number(activeCell?.dataset.assessmentBaseReturn);
                    ...
                    row.querySelectorAll('.is-trigger-horizon').forEach(cell=>{
                        const changedRiskSector=Math.floor(risk/10)>Math.floor(baseRisk/10);
                        const hasDynamicRisk=netForecast<2&&risk>baseRisk+.01&&changedRiskSector;
                        cell.classList.toggle('is-time-downgraded',netForecast<2&&changedRiskSector);
                        cell.classList.toggle('has-improved-forecast',!hasDynamicRisk&&Number.isFinite(baseForecast)&&netForecast-baseForecast>=1);
                        cell.classList.toggle('has-dynamic-risk',hasDynamicRisk);
                        cell.classList.toggle('is-critical-dynamic-risk',hasDynamicRisk&&risk>=90);
                    });
                    row.querySelectorAll('[data-screener-dynamic-risk]').forEach(marker=>{
                        const riskIncreased=risk>baseRisk+.01;
                        marker.hidden=netForecast>=2;
                        marker.style.setProperty('--dynamic-risk-position',`${quality.toFixed(2)}%`);
                        marker.style.setProperty('--dynamic-risk-color',riskIncreased?(risk>=90?'#fb7185':'#fbbf24'):'#22d3ee');
                ...
                const initializeLiveSimulation=()=>{
                    ...
                    const update=()=>{
                        ...
                        const amplitude=isMobile?.015:.004;
                        prices.forEach((element,index)=>{
                            const base=Number(element.dataset.liveBasePrice);
                            if(!Number.isFinite(base)||base<=0)return;
                            const price=base*(1+(Math.sin((tick+index)*.73)*amplitude));
