...
<div id="fundamental-page" x-data="{ active: '{{ $selected ? 'ratios' : 'termine' }}' }" class="mx-auto max-w-7xl px-3 py-5 text-[var(--ak-text)] sm:px-5 lg:py-8">
    ...
    <div class="fundamental-layout">
        ...
        <section class="fundamental-main">
            @php
                // Every param that can currently be active, so every link/form
                // on this page can carry the full filter state forward -
                // picking a sector must not lose a slider drag, sorting must
                // not lose the search text, switching tabs must not lose
                // anything either, since Heatmaps/Verteilung/the table all
                // share the same underlying filtered universe.
                ...
                // reload doesn't snap the handle back to "no filter" while
                // the chart itself stays filtered.
                $initialThresholds = [];
            ...
            <div x-show="active === 'heatmaps' || active === 'verteilung'" x-cloak>
                <div class="mb-3 flex items-start gap-1.5">
                    <p class="text-xs font-semibold text-[var(--ak-muted)]">{{ __('Verteilung der gesamten Aktien-Universum über die 4 Kernkennzahlen. Regler filtern ab welchem Dezil hervorgehoben bleibt. Klick auf eine Aktie in der Liste öffnet ihre eigene Detailseite.') }}</p>
                    <div class="fundamental-help" x-data="{ open: false }" @keydown.escape.window="open = false">
                        <button type="button" class="fundamental-help-btn" @click="open = !open" :aria-expanded="open" aria-label="{{ __('Hilfe: Kennzahlen erklärt') }}">
                        ...
                        <div class="fundamental-help-backdrop" x-show="open" x-cloak x-transition.opacity @click="open = false">
                            <div class="fundamental-help-panel" @click.stop>
                                ...
                                <h4>{{ __('Heatmaps & Verteilung benutzen') }}</h4>
                                <p>{{ __('Heatmaps zeigen je zwei Kennzahlen gegeneinander (10x10 Dezile), Verteilung zeigt jede Kennzahl einzeln als Balkendiagramm (10 Dezile) - beide Ansichten teilen dieselbe Aktienauswahl und Filter.') }}</p>
                                ...
                                <p>{{ __('Ein Regler wirkt auf alle Heatmaps und Balkendiagramme gleichzeitig, auch auf Panels, die die gezogene Kennzahl gar nicht selbst zeigen - betroffene Zellen/Balken werden abgedunkelt statt ausgeblendet, damit die Gesamtstruktur sichtbar bleibt.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div
                     x-data="{
                        ...
                        pctFromEvent(e, dragging) {
                            const r = this.$refs[(dragging.kind === 'dist' ? 'dplot' : 'hplot') + dragging.panel].getBoundingClientRect();
                            const p = dragging.axis === 'y' ? (r.bottom - e.clientY) / r.height : (e.clientX - r.left) / r.width;
                            return Math.min(9, Math.max(0, Math.round(Math.min(1, Math.max(0, p)) * 10)));
                        },
                        onMove(e) {
                            if (!this.dragging) return;
                            let v = this.pctFromEvent(e, this.dragging);
                            // For an inverted metric the line renders one bucket
                            // past the threshold (see the *DragStyle vars in the
                            // blade below), so the achievable line positions are
                            // shifted right by one bucket-width relative to the
                            // raw pointer-to-bucket reading - shift the detected
                            // bucket back by one to match, or the line would
                            // visibly lag the cursor while dragging.
                    ...
                    <section class="ak-heatmap-metric-grid grid w-full grid-cols-1 items-start gap-3 md:grid-cols-2 xl:grid-cols-4" x-show="active === 'heatmaps'" x-cloak>
                            ...
                            <div class="fundamental-heatmap-card flex h-auto min-w-0 flex-col rounded-2xl border border-[var(--ak-border)] bg-[var(--ak-card)] p-3 pb-4 shadow-[var(--ak-shadow)]">
                                ...
                                <div class="flex gap-1">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div x-ref="hplot{{ $i }}" class="fundamental-heatmap-plot relative grid aspect-square h-auto w-full grid-cols-10 grid-rows-10 gap-[3px]">
                                            @for($yb = 9; $yb >= 0; $yb--)
                                                @for($xb = 0; $xb <= 9; $xb++)
                                                    @php
                                            ...
                                            <span class="fundamental-heatmap-drag fundamental-heatmap-drag--x" :style="`{{ $xDragStyle }}`" @pointerdown="dragging = { metric: '{{ $panel['x_key'] }}', axis: 'x', panel: {{ $i }}, kind: 'heatmap' }">
                                                <b></b><i></i>
                                            </span>
                                            <span class="fundamental-heatmap-drag fundamental-heatmap-drag--y" :style="`{{ $yDragStyle }}`" @pointerdown="dragging = { metric: '{{ $panel['y_key'] }}', axis: 'y', panel: {{ $i }}, kind: 'heatmap' }">
                                                <b></b><i></i>
                                            </span>
                                        </div>
                    ...
                    <section class="fundamental-dist-grid" x-show="active === 'verteilung'" x-cloak>
                        @foreach($histograms as $i => $m)
                            ...
                            <div class="fundamental-dist-card">
                                ...
                                <div x-ref="dplot{{ $i }}" class="fundamental-dist-plot">
                                    @for($b = 0; $b <= 9; $b++)
                                        @php
                                            $count = $m['counts'][$b];
                                    ...
                                    @endfor
                                    <span class="fundamental-heatmap-drag fundamental-heatmap-drag--x" :style="`{{ $dragStyle }}`" @pointerdown="dragging = { metric: '{{ $m['key'] }}', axis: 'x', panel: {{ $i }}, kind: 'dist' }">
                                        <b></b><i></i>
                                    </span>
                                </div>
                                ...
                                <p class="mt-1.5 text-[9px] font-bold text-[var(--ak-muted)]">
                                    {{ __('Filter:') }} <b class="text-[var(--ak-text)]" x-text="{{ $distThresholdExpr }}"></b>
                                </p>
                            </div>
                        @endforeach
                    </section>
            ...
            <div class="ak-master-card fundamental-section" x-show="active === 'termine'" x-cloak>
                ...
                <div class="ak-master-card-header fundamental-head-inner">
                    <h2 class="text-base font-black">{{ __('Alle Termine') }}</h2>
                    <p class="mt-1 text-[10px] font-semibold text-[var(--ak-muted)]">{{ __(':n Quartalstermine bis :date angekündigt. Datenabdeckung ist lückenhaft (Feed deckt nicht das gesamte Aktien-Universum ab).', ['n' => $calendarData['total'], 'date' => $calendarData['range_end']->format('d.m.Y')]) }}</p>
                </div>
                ...
                <div class="calendar-list">
                    @forelse($calendarData['events'] as $event)
                        <a href="{{ route('fundamental.index', ['symbol' => $event['symbol']]) }}" class="calendar-row">
...
<style>
    ...
    /* Same tile pattern as the Concept Dashboard's left icon grid
       (#dashboard-concept-page .concept-icon-grid/.concept-icon-tile): a
       small, fixed set of view sections, swapped client-side with Alpine. */
    ...
    #fundamental-page .fundamental-dist-grid { display: grid; grid-template-columns: 1fr; gap: .9rem; }
    @media (min-width: 700px) { #fundamental-page .fundamental-dist-grid { grid-template-columns: repeat(2, 1fr); } }
    @media (min-width: 1300px) { #fundamental-page .fundamental-dist-grid { grid-template-columns: repeat(4, 1fr); } }
    #fundamental-page .fundamental-dist-card { border: 1px solid var(--ak-border); background: var(--ak-card); border-radius: 1rem; padding: .9rem; box-shadow: var(--ak-shadow); }
    #fundamental-page .fundamental-dist-plot { position: relative; display: flex; align-items: flex-end; gap: 3px; height: 140px; }
    ...
    #fundamental-page .calendar-chart { display: flex; align-items: flex-end; gap: 3px; padding: 1.1rem 1.1rem 1.6rem; overflow-x: auto; }
    #fundamental-page .calendar-week { position: relative; flex: 1 1 0; min-width: 26px; display: flex; flex-direction: column; align-items: center; height: 130px; }
    #fundamental-page .calendar-week-bar-track { position: relative; width: 100%; height: 90px; display: flex; align-items: flex-end; justify-content: center; border-bottom: 1px dashed var(--ak-border); }
    #fundamental-page .calendar-week-bar { width: 65%; min-height: 6px; border-radius: 3px 3px 0 0; background: linear-gradient(180deg, #22d3ee, color-mix(in srgb, #22d3ee 45%, transparent)); display: flex; align-items: flex-start; justify-content: center; }
    #fundamental-page .calendar-week-count { margin-top: -14px; font-size: .6rem; font-weight: 900; color: var(--ak-text); }
    #fundamental-page .calendar-week-label { margin-top: .35rem; font-size: .58rem; font-weight: 800; color: var(--ak-text); white-space: nowrap; }
    #fundamental-page .calendar-week-range { font-size: .52rem; font-weight: 600; color: var(--ak-muted); white-space: nowrap; }
    #fundamental-page .calendar-week.is-current .calendar-week-label { color: #22d3ee; }
    #fundamental-page .calendar-week.is-current .calendar-week-bar-track { border-bottom-color: #22d3ee; }
    #fundamental-page .calendar-list { display: flex; flex-direction: column; }
    #fundamental-page .calendar-row { display: flex; align-items: center; gap: 1rem; padding: .7rem 1.1rem; border-top: 1px solid var(--ak-border); text-decoration: none; color: inherit; transition: background .15s ease; }
    #fundamental-page .calendar-row:first-child { border-top: none; }
    #fundamental-page .calendar-row:hover { background: color-mix(in srgb, #22d3ee 6%, transparent); }
    #fundamental-page .calendar-row-date { flex: 0 0 48px; text-align: center; }
    #fundamental-page .calendar-row-date b { display: block; font-size: .78rem; font-weight: 900; }
    #fundamental-page .calendar-row-date span { display: block; font-size: .58rem; font-weight: 700; color: var(--ak-muted); text-transform: uppercase; }
    #fundamental-page .calendar-row-symbol { flex: 0 0 72px; font-size: .78rem; font-weight: 900; color: #22d3ee; }
    #fundamental-page .calendar-row-main { flex: 1 1 auto; display: flex; flex-wrap: wrap; align-items: baseline; gap: .5rem; min-width: 0; }
    #fundamental-page .calendar-row-name { font-size: .74rem; font-weight: 600; color: var(--ak-text); }
    #fundamental-page .calendar-row-sector { font-size: .62rem; font-weight: 700; color: var(--ak-muted); }
    #fundamental-page .calendar-row-countdown { flex: 0 0 auto; font-size: .62rem; font-weight: 800; color: var(--ak-muted); padding: .25rem .6rem; border: 1px solid var(--ak-border); border-radius: .6rem; white-space: nowrap; }
