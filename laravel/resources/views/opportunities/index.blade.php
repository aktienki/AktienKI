            ...
            @forelse($opportunities as $opportunity)
                ...
                <article class="overflow-hidden rounded-2xl border border-[var(--ak-border)] border-l-4 border-l-cyan-400 bg-transparent shadow-[var(--ak-shadow)]">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        <form method="POST" action="{{ route('opportunities.update', $opportunity) }}">@csrf @method('PATCH')<input type="hidden" name="notify_on_buy" value="{{ $opportunity->notify_on_buy ? 0 : 1 }}"><button class="inline-flex h-10 items-center gap-2 rounded-xl border border-amber-400/35 bg-amber-400/10 px-4 text-xs font-black text-amber-400"><x-heroicon-o-bell class="h-4 w-4" />{{ $opportunity->notify_on_buy ? __('POSITIV-Erinnerung aktiv') : __('Bei POSITIV erinnern') }}</button></form>
