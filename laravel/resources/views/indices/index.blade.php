...
<div id="aggregate-screener" class="index-screener screener-page mx-auto max-w-7xl px-3 py-5 text-[var(--ak-text)] sm:px-5 lg:py-8">
    ...
    @foreach($indices as $index)
        ...
        @endphp
        <x-screener.aggregate-card :rank="$index->global_rank" :eyebrow="__('Globales Ranking')" :name="$index->name" :symbol="$index->symbol" :meta="$index->country.' · '.$index->currency" :secondary-meta="$index->region" :members="(int)$index->members_count" :analyzed="(int)$index->analyzed_count" :score="$index->rating_value" :confidence="$confidence" :hit-rate="$hitRate" :profit-per-trade="$profitPerTrade" :stability="$stability" :risk="$risk" :expected-return="$return" :description="$index->description" :assessment="$assessment" :chart-points="$index->chart_points" :chart-label="__('Indexverlauf · 1 Jahr')" :top-stocks="$index->top_stocks" :score-trend="$index->score_trend" :realtime-quotes="$realtimeQuotes" :market-info="$marketInfo" :market-info-date="$dailyInfo?->analysis_date" :market-info-model="$dailyInfo?->model" :target="route('stocks.index',['index'=>$index->symbol])" mobile-compact />
    @endforeach
    </section></div>
</div>
<style>#aggregate-screener{height:calc(100dvh - 73px);display:flex;min-height:0;flex-direction:column;overflow:hidden}#aggregate-screener>header,#aggregate-screener>.screener-filter-shell,#aggregate-screener>div.mb-3{flex:0 0 auto}.aggregate-results{min-height:0;flex:1 1 auto;overflow:auto;overscroll-behavior:contain;padding-right:3px}@media(min-width:1536px){.index-screener .screener-stock-card{height:20rem}}@media(min-width:1024px) and (max-width:1535px){.index-screener .screener-stock-card{height:auto;min-height:38rem}}@media(max-width:1023px){#aggregate-screener{height:auto;overflow:visible}.aggregate-results{overflow:visible}}</style>
@if($realtimeQuotes)
<script>
