...
<head>
    ...
    <title>Signalbericht · {{ $instrument['symbol'] ?? $report->symbol }}</title>
    <style>
...
<main class="shell">
    <header class="hero">
        <div class="eyebrow">Aktuelles Signal · Offline-Bericht</div>
        <div class="actions"><a class="btn primary" href="{{ route('analysis-reports.pdf', $report) }}" target="_blank" rel="noopener">PDF öffnen ↗</a></div>
        <h1>{{ $instrument['symbol'] ?? $report->symbol }} · {{ $instrument['name'] ?? 'Aktie' }}</h1>
        <div class="sub">{{ $instrument['country'] ?? '—' }} · {{ $instrument['sector'] ?? '—' }} · Erstellt {{ optional($report->created_at)->format('d.m.Y H:i') }}</div>
    ...
    <div class="layout">
        <section class="card compact-card">
            <h2>Kurs & Prognose</h2><div class="muted">Historischer Verlauf mit Zielkurs des aktuellen Modells</div>
            <svg class="chart" viewBox="0 0 900 280" preserveAspectRatio="none" role="img" aria-label="Kursverlauf und Prognose">
                ...
                @if($target > 0)<line x1="24" x2="876" y1="{{ number_format($targetY,1,'.','') }}" y2="{{ number_format($targetY,1,'.','') }}" stroke="#fbbf24" stroke-width="2" stroke-dasharray="8 7"/><polygon points="850,{{ number_format($targetY,1,'.','') }} 870,{{ number_format($targetY-9,1,'.','') }} 870,{{ number_format($targetY+9,1,'.','') }}" fill="#fbbf24"/><text x="690" y="{{ number_format(max(18,$targetY-10),1,'.','') }}" fill="#fbbf24" font-size="13">20-Tage-Ziel</text>@endif
                <text class="chart-label" x="28" y="270">Historie</text><text class="chart-label" x="790" y="270">aktuell</text>
            </svg>
            <div class="rows">
                <div class="row"><span>Aktueller Kurs</span><strong>{{ $formatNumber($current['current_price'] ?? null, ' '.($instrument['currency'] ?? '')) }}</strong></div>
                <div class="row"><span>20-Tage-Zielkurs</span><strong>{{ $formatNumber($current['predicted_price_20d'] ?? null, ' '.($instrument['currency'] ?? '')) }}</strong></div>
                <div class="row"><span>Vorheriges Signal</span><strong>{{ strtoupper($transition['from'] ?? $previous['signal'] ?? '—') }}</strong></div>
            </div>
        </section>
        <aside class="card compact-card">
            <h2>Modell- und Risikokarte</h2><div class="muted">Werte zum Zeitpunkt des Signalwechsels</div>
            <div class="statgrid" style="margin-top:18px">
                <div class="stat"><b>Qualität</b><strong>{{ $current['quality_band'] ?? '—' }}</strong></div>
                <div class="stat"><b>Quality Gate</b><strong>{{ !empty($current['quality_gate_passed']) ? 'bestanden' : 'offen' }}</strong></div>
                <div class="stat"><b>Modellalter</b><strong>{{ $formatNumber($current['model_age_days'] ?? null) }} Tage</strong></div>
                <div class="stat"><b>Backtest</b><strong>{{ $current['backtest_version'] ?? '—' }}</strong></div>
            </div>
            <h3>Fundamentals</h3>
            <div class="rows">@foreach(array_slice($fundamentals, 0, 8, true) as $key => $value)<div class="row"><span>{{ str_replace('_',' ',ucwords($key)) }}</span><strong>{{ is_numeric($value) ? number_format((float)$value,2,',','.') : ($value ?: '—') }}</strong></div>@endforeach</div>
        </aside>
    </div>

    <section class="card heatmap-card">
        <div class="heatmap-head"><div><h2>Indikator-Heatmaps</h2><div class="muted">Historische Kombinationen aus Trend, Momentum und Oszillator. Die Zahl zeigt den Anteil positiver 20-Tage-Ziele.</div></div><span class="pill">{{ count($indicatorRows) }} Beobachtungen</span></div>
        <div class="heatmap-grid">
            @foreach([['Momentum 10','Stochastik %K',$momentumHeatmap,$momentumBins,$stochBins],['ADX 14','RSI 14',$adxHeatmap,$adxBins,$rsiBins]] as [$xLabel,$yLabel,$matrix,$xBins,$yBins])
                <div class="heatmap"><div class="heatmap-title"><strong>{{ $xLabel }} × {{ $yLabel }}</strong><span>↑ positive Ziele</span></div><div class="heatmap-axis"><span class="axis-y">{{ $yLabel }}</span><div class="heatmap-body"><div class="heatmap-ylabels">@foreach($yBins as $bin)<span>{{ $bin[1] === INF ? $bin[0].'+' : $bin[0].'–'.$bin[1] }}</span>@endforeach</div><div><div class="heatmap-cells">@foreach($matrix as $row)<div class="heatmap-row">@foreach($row as $cell)@php $score=$cell['score']; $hue=$score===null?210:(int)($score*1.2+175); @endphp<span class="heat-cell" style="--cell-color:hsl({{ $hue }},70%,55%)" title="{{ $cell['count'] }} Fälle">{{ $score === null ? '—' : $score.'%' }}</span>@endforeach</div>@endforeach</div><div class="heatmap-xlabels">@foreach($xBins as $bin)<span>{{ $bin[1] === INF ? $bin[0].'+' : $bin[0].'–'.$bin[1] }}</span>@endforeach</div></div></div><div class="axis-x">{{ $xLabel }}</div></div></div>
            @endforeach
        </div>
    </section>

    <section class="card ai-card"><div class="eyebrow">ChatGPT · GPT‑5.4 mini</div><h2>KI-Einschätzung & Marktimpulse</h2><div class="muted">Die Einschätzung stammt aus der KI-Abfrage und ergänzt die unveränderten Kennzahlen aus der Datenbank. News und Termine sind als Szenario gekennzeichnet.</div><div class="report-text" style="margin-top:14px">{{ strip_tags($reportText) }}</div></section>
    <div class="source">Datenbasis: aktienKI-Datenbank · Dieser Bericht dient ausschließlich Informationszwecken und ist keine Anlageberatung.</div>
</main>
</body>
</html>
