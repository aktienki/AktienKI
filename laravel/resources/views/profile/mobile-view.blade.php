<x-app-layout>
    @php
        $labels = [
            'champion' => __('Aktie des Tages'), 'personal' => __('Persönlicher Bereich'), 'community' => __('Community'),
            'market' => __('Aktives Portfolio'),
        ...
        $descriptions = [
            'champion' => __('Stärkste aktuell bewertete Aktie mit KI-Score und Risiko.'),
            'personal' => __('Dein persönlicher, aufklappbarer Bereich.'), 'community' => __('Aktivität seit deinem letzten Login.'),
            'market' => __('Aktienanzahl, Risikoprofil, KI-Score-Verteilung, Trend und Stimmung.'),
        ...
        <div class="ak-container max-w-5xl">
            ...
            <section class="ak-card overflow-hidden border-cyan-400/30">
                <header class="flex flex-wrap items-center justify-between gap-4 border-b border-cyan-400/20 p-5 sm:p-7">
                    ...
                    <span class="rounded-full border border-cyan-400/25 px-3 py-1 text-[10px] font-black text-cyan-300"><span data-mobile-card-count>{{ count($selectedCards) }}</span> {{ __('Karten aktiv') }}</span>
                </header>
                <form method="POST" action="{{ route('profile.mobile-view.update') }}" class="p-5 sm:p-7" data-mobile-card-form>
                    @csrf @method('PATCH')
                    ...
                    <button type="submit" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400 px-5 py-3 text-sm font-black text-slate-950 transition active:scale-[.99] sm:hidden" data-mobile-card-submit><x-heroicon-o-check class="h-4 w-4" />{{ __('Auswahl speichern') }}</button>
                    <div class="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($cards as $card)
                            <label class="group relative cursor-pointer touch-manipulation select-none">
                                ...
                                <span class="pointer-events-none absolute left-4 top-4 grid h-8 w-8 place-items-center rounded-lg border border-cyan-400/30 text-transparent transition peer-checked:border-cyan-300 peer-checked:bg-cyan-400 peer-checked:text-slate-950 peer-focus-visible:ring-2 peer-focus-visible:ring-cyan-300"><x-heroicon-o-check class="h-4 w-4" /></span>
                            </label>
                        @endforeach
                    </div>
                    @error('cards')<p class="mt-3 text-sm font-bold text-rose-300" data-mobile-card-error>{{ $message }}</p>@else<p class="mt-3 hidden text-sm font-bold text-rose-300" data-mobile-card-error>{{ __('Wähle mindestens eine Karte aus.') }}</p>@enderror
                    <footer class="mt-7 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--ak-border)] pt-5">
                        ...
                        <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400 px-5 py-3 text-sm font-black text-slate-950 transition hover:bg-cyan-300 sm:w-auto" data-mobile-card-submit><x-heroicon-o-check class="h-4 w-4" />{{ __('Mobile Ansicht speichern') }}</button>
                    </footer>
                </form>
    ...
    <script>
        (() => {
            const form = document.querySelector('[data-mobile-card-form]');
            const count = document.querySelector('[data-mobile-card-count]');
            const error = form?.querySelector('[data-mobile-card-error]');
            if (!form || !count) return;

            const selectedCards = () => form.querySelectorAll('input[name="cards[]"]:checked');
            const refreshCount = () => {
            ...
            form.addEventListener('submit', (event) => {
                ...
                form.querySelectorAll('[data-mobile-card-submit]').forEach((button) => {
