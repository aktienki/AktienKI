    ...
    <main class="mx-auto max-w-5xl space-y-6 px-5 py-8 sm:px-8">
        <header class="ak-card rounded-2xl border border-amber-400/35 bg-[var(--ak-surface)] p-6 shadow-xl">
            ...
            <div class="mt-2 flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-black tracking-tight">{{ __('Beta-Einladungscodes') }}</h1>
                    <p class="mt-1 text-sm text-[var(--ak-muted)]">{{ __('Erstelle Codes mit optionalem Ablaufdatum und begrenzter Nutzung für ausgewählte Tester.') }}</p>
                </div>
        ...
        @if ($generatedUrl)
            <section class="rounded-2xl border border-amber-400/50 bg-amber-400/10 p-5">
                <p class="text-xs font-black uppercase tracking-[.16em] text-amber-300">{{ __('Code jetzt kopieren') }}</p>
                <p class="mt-2 rounded-lg border border-amber-300/30 bg-black/20 px-3 py-3 text-center font-mono text-2xl font-black tracking-[.16em] text-amber-200">{{ $generatedToken }}</p>
                <p class="mt-3 text-xs font-black uppercase tracking-[.16em] text-amber-300">{{ __('Alternativ: Einladungslink kopieren') }}</p>
                <p class="mt-2 break-all rounded-lg bg-black/20 px-3 py-2 font-mono text-sm text-white">{{ $generatedUrl }}</p>
                <p class="mt-2 text-xs text-amber-100/80">{{ __('Aus Sicherheitsgründen wird der Einladungscode nur in diesem Browser-Schritt angezeigt.') }}</p>
            </section>
        @endif

        <section class="ak-card rounded-2xl border border-[var(--ak-border)] bg-[var(--ak-surface)] p-6">
            <h2 class="text-lg font-black">{{ __('Neuen Einladungscode erstellen') }}</h2>
            <form method="POST" action="{{ route('beta.invitations.store') }}" class="mt-4 grid gap-4 sm:grid-cols-3">
                ...
                <button class="rounded-lg bg-amber-400 px-5 py-3 font-black text-slate-950 transition hover:bg-amber-300 sm:col-span-3 sm:justify-self-start">{{ __('Einladungscode generieren') }}</button>
            </form>
        </section>

        <section class="ak-card overflow-hidden rounded-2xl border border-[var(--ak-border)] bg-[var(--ak-surface)]">
            <div class="border-b border-[var(--ak-border)] px-6 py-4"><h2 class="font-black">{{ __('Bestehende Einladungscodes') }}</h2></div>
