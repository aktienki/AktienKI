        ...
        <section class="auth-shell grid w-full max-w-6xl overflow-hidden rounded-2xl border lg:grid-cols-[1.05fr_.95fr]" x-data="{ step: {{ $initialStep }}, riskLevel: @js(old('risk_level', 'normal')), disclaimerAccepted: @js((bool) old('accept_disclaimer')) }">
            <div class="p-6 sm:p-8 lg:p-8">
                ...
                <form id="registration-form" method="POST" action="{{ route('register') }}" class="mt-5">
                    ...
                    <div x-cloak x-show="step === 2" data-step="2">
                        ...
                        <fieldset role="radiogroup" aria-label="{{ __('Gewünschtes Risikoprofil') }}">
                            ...
                            <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                                ...
                                <button type="button" role="radio" :aria-checked="riskLevel === 'risk'" @click="riskLevel = 'risk'" class="h-full rounded-lg border p-3 text-left transition" :class="riskLevel === 'risk' ? 'border-red-400/70 bg-red-400/[.12] ring-2 ring-red-400/15' : 'border-red-400/15 bg-[#0b1830]/45 hover:border-red-400/40'"><span class="block text-xs font-bold text-red-400">{{ __('Risk') }}</span><span class="mt-1 block text-[10px] leading-4 text-slate-400">{{ __('Vollständiges Universum einschließlich SLEEP-Aktien mit Profitfaktor unter 1,05.') }}</span></button>
                            </div>
                    ...
                    <div class="mt-3 grid grid-cols-[.7fr_1.3fr] gap-3"><button type="button" @click="step = 1" class="h-11 rounded-lg border border-orange-400/15 font-bold text-slate-300 hover:bg-orange-400/[.06]">{{ __('Zurück') }}</button><button type="submit" class="h-11 rounded-lg border border-orange-400/45 bg-orange-400/25 font-bold text-orange-300 shadow-lg shadow-orange-400/20 transition hover:border-orange-300/65 hover:bg-orange-400/35 hover:text-white">{{ __('Jetzt registrieren') }}</button></div>
                    </div>
                </form>
