...
@php
    ...
    $legacyFixedSizing = ($summary['source_type'] ?? null) === 'serving_model_configurations'
        && empty($summary['allocation_mode'])
        && is_numeric($summary['position_notional'] ?? null);
@endphp
...
@if(($summary['source_type'] ?? null) === 'serving_model_configurations' && !empty($summary['model_references']))
<h2>Abgleich mit der Modellberechnung</h2>
<div class="notice">Kapitalmodus: {{ $legacyFixedSizing ? 'Festbetrag '.number_format((float)$summary['position_notional'],2,',','.').' € je Kauf' : ($summary['allocation_mode_label'] ?? 'Balancing') }} · maximal {{ (int)($summary['maximum_positions'] ?? 5) }} gleichzeitig gehaltene Aktien@if(!$legacyFixedSizing && ($summary['allocation_mode'] ?? null) === \App\Services\ServingPortfolioCalculator::ALLOCATION_EQUAL_WEIGHT) · maximal {{ number_format((float)($summary['max_stock_allocation_percent'] ?? 30),0,',','.') }} % Depotanteil je Aktie@endif · 0,3 % Kosten je Order, mindestens 10,00 €.</div>
<table class="data compact-data"><thead><tr><th>Konfiguration</th><th>Zeitraum</th><th>Trades</th><th>Trefferquote</th><th>Profitfaktor</th><th>Modellrendite</th><th>Modell-Drawdown</th></tr></thead><tbody>
@foreach($summary['model_references'] as $reference)
<tr><td>{{ $reference['symbol'] }} · {{ $reference['horizon_days'] }}T · {{ $reference['variant_label'] }}</td><td>{{ $reference['start_date'] }} – {{ $reference['end_date'] }}</td><td>{{ $reference['trades'] }}</td><td>{{ number_format((float)$reference['hit_rate_percent'],2,',','.') }} %</td><td>{{ is_numeric($reference['profit_factor'] ?? null) ? number_format((float)$reference['profit_factor'],3,',','.') : '—' }}</td><td class="{{ (float)$reference['cumulative_return_percent'] >= 0 ? 'buy' : 'sell' }}">{{ (float)$reference['cumulative_return_percent'] >= 0 ? '+' : '' }}{{ number_format((float)$reference['cumulative_return_percent'],2,',','.') }} %</td><td>{{ number_format((float)$reference['max_drawdown_percent'],2,',','.') }} %</td></tr>
@endforeach
</tbody></table>
<div class="notice">Die Modellrendite nutzt – identisch zur Modellseite – die vollständige Wiederanlage der gespeicherten Nettoerträge. @if($legacyFixedSizing)Dieser ältere Depotlauf verwendet noch einen festen Betrag je Kauf.@elseif(($summary['allocation_mode'] ?? null) === \App\Services\ServingPortfolioCalculator::ALLOCATION_FULL_INVESTMENT)Im Vollinvestitionsmodus wird das gesamte freie Kapital abzüglich Orderkosten eingesetzt.@else Beim Balancing wird das Kapital auf die gewählte Aktienzahl verteilt und zusätzlich durch den Maximalanteil je Aktie begrenzt.@endif Deshalb unterscheiden sich die Renditen, obwohl dieselben Trades zugrunde liegen.</div>
@endif
...
@endif
<h2>Transaktionen</h2>
@if(($summary['source_type'] ?? null) === 'serving_model_configurations')
<table class="data compact-data"><thead><tr><th>Datum</th><th>Aktion</th><th>Aktie</th><th>Signal</th><th>Modell</th><th>Horizont</th><th>Stück</th><th>Kurs</th><th>Modelltrade</th><th>Depot-P/L</th><th>Gebühr</th></tr></thead><tbody>
@foreach($transactions as $transaction)@php $stat = $transactionRows[$transaction->id] ?? []; @endphp<tr><td>{{ $transaction->transaction_date?->format('d.m.Y') }}</td><td class="{{ $transaction->type==='sell'?'sell':'buy' }}">{{ strtoupper($transaction->type) }}</td><td>{{ $transaction->instrument?->symbol }}</td><td>{{ $stat['signal'] ?? 'BUY' }}</td><td>{{ $stat['variant_label'] ?? '—' }}</td><td>{{ isset($stat['horizon_days']) ? $stat['horizon_days'].'T' : '—' }}</td><td>{{ number_format((float)$transaction->quantity,4,',','.') }}</td><td>{{ number_format((float)$transaction->price,4,',','.') }}</td><td>{{ isset($stat['model_net_return']) && is_numeric($stat['model_net_return']) ? (((float)$stat['model_net_return'] >= 0 ? '+' : '').number_format((float)$stat['model_net_return'],2,',','.').' %') : '—' }}</td><td class="{{ isset($stat['pnl']) && (float)$stat['pnl'] >= 0 ? 'buy' : 'sell' }}">{{ isset($stat['pnl']) && is_numeric($stat['pnl']) ? number_format((float)$stat['pnl'],2,',','.') . ' ' . $portfolio->currency : '—' }}</td><td>{{ number_format((float)$transaction->fees,2,',','.') }}</td></tr>@endforeach
@else
<table class="data compact-data"><thead><tr><th>Datum</th><th>Aktion</th><th>Aktie</th><th>Stück</th><th>Kurs</th><th>KI-Score</th><th>Konfidenz</th><th>Risiko</th><th>Gewinn / Verlust</th><th>Gebühr</th><th>Strategien</th></tr></thead><tbody>
@foreach($transactions as $transaction)@php $stat = $transactionRows[$transaction->id] ?? []; @endphp<tr><td>{{ $transaction->transaction_date?->format('d.m.Y') }}</td><td class="{{ $transaction->type==='sell'?'sell':'buy' }}">{{ strtoupper($transaction->type) }}</td><td>{{ $transaction->instrument?->symbol }}</td><td>{{ number_format((float)$transaction->quantity,4,',','.') }}</td><td>{{ number_format($transaction->price,2,',','.') }}</td><td>{{ isset($stat['ki_score']) && is_numeric($stat['ki_score']) ? number_format((float)$stat['ki_score'],1,',','.') : '—' }}</td><td>{{ isset($stat['confidence']) && is_numeric($stat['confidence']) ? number_format((float)$stat['confidence'],1,',','.') : '—' }}{{ isset($stat['confidence']) && is_numeric($stat['confidence']) ? ' %' : '' }}</td><td>{{ isset($stat['risk']) && is_numeric($stat['risk']) ? number_format((float)$stat['risk'],1,',','.') : '—' }}</td><td class="{{ isset($stat['pnl']) && (float)$stat['pnl'] >= 0 ? 'buy' : 'sell' }}">{{ isset($stat['pnl']) && is_numeric($stat['pnl']) ? number_format((float)$stat['pnl'],2,',','.') . ' ' . $portfolio->currency : '—' }}</td><td>{{ number_format($transaction->fees,2,',','.') }}</td><td>{{ collect(data_get($transaction->meta,'strategy_ids',[data_get($transaction->meta,'strategy_id')]))->filter()->join(', ') }}</td></tr>@endforeach
@endif
