<x-app-layout>
    <div x-data="{ simulationOpen: {{ request()->boolean('test') && !$liveSimulationEnabled ? 'true' : 'false' }}, simulationSubmitting: false, automationOpen: false, strategyConfirmOpen: false, capitalOpen: false, resetOpen: false, deleteOpen: false }" class="flex min-h-[calc(100dvh-89px)] flex-col py-4 text-[var(--ak-text)]">
        <div class="mb-4 flex shrink-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <div class="mt-1 flex items-center gap-2"><h1 class="truncate text-2xl font-black tracking-tight">{{ $portfolio->name }}</h1>@if($liveSimulationEnabled)<span class="inline-flex shrink-0 items-center gap-1 rounded-md border border-cyan-300/25 bg-cyan-400/[.09] px-2 py-1 text-[9px] font-black uppercase tracking-wide text-cyan-200"><span class="h-1.5 w-1.5 rounded-full bg-cyan-300"></span>{{ __('Strategie') }}</span>@endif</div>
            ...
            @if($portfolio->type === 'paper')<button type="button" @click="resetOpen=true" class="inline-flex h-10 items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-400/[.07] px-3 text-xs font-black text-cyan-200"><x-heroicon-o-arrow-path class="h-4 w-4" />{{ __('Zurücksetzen') }}</button>@endif
            ...
            <div class="h-full overflow-hidden rounded-2xl border border-[var(--ak-border)] bg-[var(--ak-card)] p-3 shadow-[var(--ak-shadow)]">
                ...
                <div class="grid grid-cols-2 gap-x-4 xl:grid-cols-3">
                    @foreach ([
                        ...
                        [__('Trefferquote'), number_format((float) ($simulationSummary['hit_rate_percent'] ?? 0), 1, ',', '.').' %', 'text-cyan-300'],
                        [__('Profitfaktor'), number_format((float) ($simulationSummary['profit_factor'] ?? 0), 2, ',', '.'), 'text-cyan-300'],
                        [__('Max. Drawdown'), number_format((float) ($simulationSummary['max_drawdown_percent'] ?? 0), 1, ',', '.').' %', 'text-rose-300'],
                        [__('Verschiedene Aktien'), $distinctStocksCount, 'text-cyan-300'],
                    ...
                    <div class="mt-1 border-t border-[var(--ak-border)] pt-2">
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            @foreach($portfolio->strategies as $strategy)
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-1.5 rounded-md border border-cyan-300/20 bg-cyan-400/[.07] px-2 py-1 text-[9px] font-black text-cyan-200"><span class="max-w-28 truncate">{{ $strategy->name }}</span><strong class="tabular-nums text-white">{{ number_format($strategyShare, 0, ',', '.') }} %</strong></span>
                            @endforeach
                        </div>
                    </div>
                @endif
        ...
        <div x-show="automationOpen" x-cloak class="fixed inset-0 z-[125] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-sm" @keydown.escape.window="automationOpen=false">
            <form method="POST" action="{{ route('depots.automation.update', $portfolio) }}" class="w-full max-w-lg rounded-2xl border border-cyan-300/25 bg-[#16253a]/90 p-6 shadow-2xl" @click.outside="automationOpen=false">
                @csrf @method('PUT')
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-400/10 text-cyan-300"><x-heroicon-o-bolt class="h-7 w-7" /></div>
                ...
                <label class="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-cyan-300/20 bg-cyan-400/[.06] p-3 text-sm font-bold text-cyan-100"><input type="checkbox" name="transaction_email_enabled" value="1" @checked($transactionEmailsEnabled) class="mt-0.5 h-4 w-4 rounded border-cyan-300/40 bg-slate-950 text-cyan-500 focus:ring-cyan-500/30"><span><strong class="block text-white">{{ __('E-Mail pro Transaktion') }}</strong><small class="mt-1 block font-medium text-slate-300">{{ __('Bei jedem simulierten Kauf oder Verkauf eine E-Mail senden.') }}</small></span></label>
                ...
                <div class="mt-5 flex justify-end gap-2">
                    ...
                    <button type="submit" name="enabled" value="1" class="h-10 rounded-lg border border-cyan-300/25 bg-cyan-400/[.12] px-4 text-xs font-black text-cyan-100">{{ $liveSimulationEnabled ? __('Einstellungen speichern') : __('Konto aktivieren') }}</button>
                </div>
            </form>
        </div>
        ...
        <div x-show="strategyConfirmOpen" x-cloak class="fixed inset-0 z-[121] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-sm" @keydown.escape.window="strategyConfirmOpen=false">
            <div class="w-full max-w-lg rounded-2xl border border-cyan-300/25 bg-[#16253a]/90 p-6 shadow-2xl" @click.outside="strategyConfirmOpen=false">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-400/10 text-cyan-300"><x-heroicon-o-adjustments-horizontal class="h-7 w-7" /></div>
    ...
    <style>
      ...
      .ak-portfolio-line-chart path.apexcharts-line{filter:drop-shadow(0 0 2px rgba(34,211,238,.62)) drop-shadow(0 0 5px rgba(34,211,238,.2))}
    ...
    document.addEventListener('DOMContentLoaded',()=>{
      ...
      if(window.ApexCharts&&chartNode&&curve.length){
        ...
        const profitBars=trades.filter(trade=>trade.sell_date&&Number.isFinite(Number(trade.realized_profit))).map(trade=>({x:new Date(trade.sell_date).getTime(),y:Number(trade.realized_profit),trade,fillColor:Number(trade.realized_profit)>=0?'#2dd4bf':'#fb7185'}));
        ...
        const yearBadges=yearlyData.map(([year,points])=>{
          ...
          return {
            ...
            label:{
              ...
              offsetY:2,
              borderColor:positive?'rgba(45,212,191,.38)':'rgba(251,113,133,.38)',
              style:{background:positive?'rgba(13,148,136,.22)':'rgba(190,24,93,.2)',color:positive?'#99f6e4':'#fecdd3',fontSize:'8px',fontWeight:800,padding:{left:5,right:5,top:2,bottom:2}},
            },
          };
        });
        new ApexCharts(chartNode,{chart:{type:'line',height:chartHeight,toolbar:{show:false},animations:{enabled:false},zoom:{enabled:false}},series,colors:['#22d3ee'],stroke:{show:true,width:2.4,curve:'smooth',lineCap:'round'},markers:{size:0,hover:{sizeOffset:0}},dataLabels:{enabled:false},legend:{show:false},annotations:{xaxis:[...yearBoundaries,...yearBadges]},xaxis:{type:'datetime',min:chartMin,max:chartMax+rightOffset,labels:{show:true,datetimeUTC:false,format:'dd.MM.yy',style:{colors:'#7f93a8',fontSize:'8px'},hideOverlappingLabels:true},axisBorder:{show:true,color:'rgba(148,163,184,.18)'},axisTicks:{show:false},tooltip:{enabled:false}},yaxis:{show:true,min:chartValueMin,max:chartValueMax,forceNiceScale:false,decimalsInFloat:0,labels:{show:true,minWidth:34,style:{colors:'#7f93a8',fontSize:'8px'},formatter:value=>`${moneyLabel(value)} {{ $portfolio->currency }}`},axisBorder:{show:false}},grid:{borderColor:'rgba(148,163,184,.12)',padding:{top:20,bottom:0,left:2,right:10}},theme:{mode:'dark'},tooltip:{shared:false,intersect:true,custom:({seriesIndex,dataPointIndex,w})=>{
        ...
        if(profitNode&&profitBars.length){
          ...
          profitBars.forEach(point=>{
            ...
            bar.style.cssText=`position:absolute;left:${position}%;width:4px;height:${height}px;background:${point.y>=0?'#2dd4bf':'#fb7185'};opacity:.62;border-radius:1px;${point.y>=0?'bottom:50%':'top:50%'}`;
