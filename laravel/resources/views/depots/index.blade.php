<x-app-layout>
    ...
    <div x-data="depotExplorer(@js($depotExplorerData))" class="ak-detail-design flex min-h-[calc(100dvh-89px)] flex-col py-3 text-[var(--ak-text)]">
        <div class="ak-detail-hero mb-3 flex shrink-0 flex-col gap-2 rounded-2xl border border-[var(--ak-border)] px-4 py-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <a x-show="phase !== 'detail'" href="{{ route('dashboard') }}" data-back-link class="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl border border-[var(--ak-border)] bg-[var(--ak-card)] px-4 text-xs font-black text-[var(--ak-muted)] transition hover:border-[var(--ak-border-strong)] hover:text-[var(--ak-text)]">
                <x-heroicon-o-arrow-left class="h-4 w-4" />{{ __('Zurück') }}
            </a>
        </div>
                ...
                <article x-data="{ strategyOpen: false, strategyConfirmOpen: false, capitalOpen: false, resetOpen: false, deleteOpen: false, deleteConfirmed: false, testPlanOpen: false }" x-show="!selected" x-transition.opacity.duration.1000ms class="ak-detail-panel relative flex flex-col overflow-hidden rounded-2xl border bg-[var(--ak-card)] transition {{ $isStrategyAccount ? 'min-h-[29rem] border-orange-400/50 shadow-[0_0_0_1px_rgba(251,146,60,.08),0_18px_45px_rgba(251,146,60,.16)] xl:row-span-2' : 'min-h-52 border-[var(--ak-border)] shadow-[var(--ak-shadow)] hover:border-teal-500/35' }}" @if($isStrategyAccount) style="background:linear-gradient(155deg,rgba(251,146,60,.14) 0%,rgba(21,36,58,.96) 34%,rgba(21,36,58,1) 100%);" @endif>
                        ...
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                @if($isStrategyAccount)
                                    <p class="text-[9px] font-black uppercase tracking-[.14em] text-orange-400">{{ __('Aktives Strategiedepot') }}</p>
                            ...
                            @else
                                <div class="flex flex-wrap gap-1.5">
                                    @foreach($portfolio->strategies as $strategy)
                                        <span class="inline-flex items-center gap-1.5 rounded-md border border-orange-400/20 bg-orange-400/[.07] px-2.5 py-1 text-[10px] font-black text-orange-400" title="{{ __('Priorität') }} {{ $strategy->pivot->priority }}">
                                            <span class="h-1.5 w-1.5 rounded-full bg-orange-400"></span>{{ $strategy->name }}
                                        </span>
                                    @endforeach
                                </div>
                            @endif
                    ...
                    <div class="mt-auto flex flex-wrap items-center justify-between gap-2 border-t border-[var(--ak-border)] {{ $isStrategyAccount ? 'px-4 py-3' : 'px-3 py-2' }}">
                        ...
                        @if($portfolio->type === 'paper' && $canEditPortfolio)
                            ...
                            <button type="button" @click="deleteConfirmed=false; deleteOpen=true" title="{{ __('Musterdepot löschen') }}" aria-label="{{ __('Musterdepot löschen') }}" class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-rose-300/20 bg-rose-400/[.07] text-rose-300"><x-heroicon-o-trash class="h-4 w-4" /></button>
                        @endif
                        ...
                        @if($portfolio->type === 'paper')
                            ...
                            <a href="{{ route('depots.show', ['portfolio' => $portfolio, 'return_to' => request()->getRequestUri(), 'test' => 1]) }}" class="inline-flex h-9 items-center gap-2 rounded-xl border border-cyan-400/30 bg-cyan-400/[.09] px-3 text-xs font-black text-cyan-200 shadow-[0_0_16px_rgba(34,211,238,.08)] transition hover:border-cyan-300/45 hover:bg-cyan-400/[.15]">
                            ...
                            @endif
                        @endif
                        <a href="{{ route('depots.show', ['portfolio' => $portfolio, 'return_to' => request()->getRequestUri()]) }}" class="inline-flex h-9 items-center gap-2 rounded-xl border border-[var(--ak-border)] bg-[var(--ak-surface-muted)] px-3 text-xs font-black text-[var(--ak-muted)] transition hover:border-teal-500/35 hover:text-teal-700">
                    ...
                    @if($portfolio->type === 'paper')
                        <div x-show="strategyOpen" x-cloak class="fixed inset-0 z-[130] grid place-items-center bg-slate-950/85 p-4 backdrop-blur-sm" @keydown.escape.window="strategyOpen=false">
                            <div class="portfolio-strategy-modal isolate w-full max-w-xl rounded-2xl border border-orange-400/25 p-6 shadow-2xl" style="background-color: rgba(22, 37, 58, 0.90);" @click.outside="strategyOpen=false">
                                <div class="flex items-start justify-between gap-4"><div><p class="portfolio-strategy-eyebrow text-[9px] font-black uppercase tracking-[.16em] text-orange-400">{{ __('Musterdepot') }}</p><h2 class="mt-1 text-xl font-black text-white">{{ __('Strategien verwalten') }}</h2><p class="portfolio-strategy-modal-subtitle mt-2 text-sm text-slate-300">{{ $portfolio->name }}</p></div><button type="button" @click="strategyOpen=false" class="portfolio-strategy-modal-close text-slate-400 hover:text-white" aria-label="{{ __('Schließen') }}"><x-heroicon-o-x-mark class="h-6 w-6" /></button></div>
                                <form id="portfolio-strategies-{{ $portfolio->id }}" method="POST" action="{{ route('depots.strategies.update', $portfolio) }}" class="mt-5 grid gap-2 sm:grid-cols-2">@csrf @method('PUT')
                                    @forelse($availableStrategies as $strategy)
                                        <label class="portfolio-strategy-option flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-slate-950/25 p-3 text-sm font-bold text-slate-200"><input type="checkbox" name="strategies[]" value="{{ $strategy->id }}" @checked($portfolio->strategies->contains('id', $strategy->id)) class="h-4 w-4 rounded border-slate-500 bg-slate-900 text-teal-500 focus:ring-teal-500/30"><span>{{ $strategy->name }}</span></label>
                                    @empty
                                ...
                                <div class="mt-5 flex justify-end gap-2"><button type="button" @click="strategyOpen=false" class="portfolio-strategy-cancel h-10 rounded-lg border border-white/10 px-4 text-xs font-black text-slate-300">{{ __('Abbrechen') }}</button><button type="button" @click="strategyOpen=false; strategyConfirmOpen=true" class="portfolio-strategy-apply h-10 rounded-lg bg-gradient-to-r from-teal-600 to-orange-400 px-4 text-xs font-black text-white">{{ __('Auswahl übernehmen') }}</button></div>
                            </div>
                        </div>
                        ...
                        <div x-show="deleteOpen" x-cloak class="ak-delete-modal fixed inset-0 z-[134] grid place-items-center bg-slate-950/70 p-4 backdrop-blur-sm" @keydown.escape.window="deleteOpen=false; deleteConfirmed=false" @click.self="deleteOpen=false; deleteConfirmed=false">
                            <form method="POST" action="{{ route('depots.destroy', $portfolio) }}" class="ak-delete-dialog isolate w-full max-w-lg rounded-2xl border p-6 shadow-2xl" @submit="if (!deleteConfirmed) $event.preventDefault()">@csrf @method('DELETE')
                                <x-heroicon-o-trash class="h-9 w-9 text-rose-500" />
                                <h2 class="mt-4 text-xl font-black text-rose-600">{{ __('Musterdepot endgültig löschen?') }}</h2>
