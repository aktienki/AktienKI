<x-app-layout>
    ...
    <div class="ak-container mx-auto max-w-[1800px] py-5">
        <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
            <div>
                <h1 class="text-3xl font-black tracking-tight text-[var(--ak-text)]">Trade Performance</h1>
        ...
        <div class="signal-history-chart-grid-layout mb-5">
            @foreach($benchmarkData as $benchmark)
                ...
                <section class="signal-history-panel ak-card ak-dashboard-card ak-card-static rounded-2xl p-4">
                    <div class="flex items-start justify-between gap-3"><div><p class="text-xs font-black uppercase tracking-[.16em] text-orange-400">Strategie vs. {{ $benchmark['label'] }}</p><p class="mt-1 text-xs text-[var(--ak-muted)]">Performance im gewählten Zeitraum</p></div><div class="text-right text-[10px] font-black"><p class="text-cyan-400">Strategie {{ $benchmark['strategy_performance'] === null ? '—' : sprintf('%+.2f %%', $benchmark['strategy_performance']) }}</p><p class="mt-1" style="color:{{ $benchmark['color'] }}">{{ $benchmark['label'] }} {{ $benchmark['performance'] === null ? '—' : sprintf('%+.2f %%', $benchmark['performance']) }}</p></div></div>
                    @if(count($benchmark['values']) > 1 && count($benchmark['strategy_values']) > 1)<svg class="signal-history-chart mt-3" viewBox="0 0 300 100" role="img" aria-label="Strategie im Vergleich zum {{ $benchmark['label'] }} über die Zeit"><line x1="8" x2="292" y1="14" y2="14" class="signal-history-chart-grid"/><line x1="8" x2="292" y1="46" y2="46" class="signal-history-chart-grid"/><line x1="8" x2="292" y1="78" y2="78" class="signal-history-chart-grid"/><line x1="8" x2="292" y1="82" y2="82" class="signal-history-chart-axis"/>@foreach($dateLabels as $dateLabel)<line x1="{{ $dateLabel['x'] }}" x2="{{ $dateLabel['x'] }}" y1="82" y2="86" class="signal-history-chart-axis"/><text x="{{ $dateLabel['x'] }}" y="96" text-anchor="{{ $loop->first ? 'start' : ($loop->last ? 'end' : 'middle') }}" class="signal-history-chart-label">{{ $dateLabel['label'] }}</text>@endforeach<polyline points="{{ $comparisonPoints($benchmark['strategy_values']) }}" class="signal-history-chart-line" stroke="#06b6d4"/><polyline points="{{ $comparisonPoints($benchmark['values']) }}" class="signal-history-chart-line" stroke="{{ $benchmark['color'] }}" stroke-width="1.2"/></svg><div class="mt-2 flex items-center gap-4 text-[9px] font-bold text-[var(--ak-muted)]"><span class="inline-flex items-center gap-1.5"><i class="h-0.5 w-5 bg-cyan-400"></i>Strategie</span><span class="inline-flex items-center gap-1.5"><i class="h-0.5 w-5" style="background:{{ $benchmark['color'] }}"></i>{{ $benchmark['label'] }}</span></div>@else<div class="mt-3 grid h-[110px] place-items-center rounded-xl bg-slate-500/5 text-xs text-[var(--ak-muted)]">Keine vergleichbaren Kursdaten</div>@endif
                </section>
            @endforeach
        </div>
        <section class="signal-history-panel ak-card ak-dashboard-card ak-card-static mb-5 overflow-hidden rounded-2xl p-4">
            ...
            @foreach([
                ...
                ['Performance pro Jahr',($stats->annual_performance > 0 ? '+' : '').number_format($stats->annual_performance,2,',','.').' %',$stats->annual_performance < 0 ? 'text-rose-400' : 'text-cyan-400'],
        ...
        <section class="signal-history-panel ak-card ak-dashboard-card ak-card-static overflow-hidden rounded-2xl">
            ...
            <div class="overflow-x-auto"><table class="min-w-[880px] w-full text-left text-xs"><thead class="text-[9px] font-black uppercase tracking-wider text-[var(--ak-muted)]"><tr><th class="px-4 py-2.5">Aktie</th><th class="px-3 py-2.5 text-right">Trades</th><th class="px-3 py-2.5 text-right">Gewinne kumulativ</th><th class="px-3 py-2.5 text-right">Verluste kumulativ</th><th class="px-3 py-2.5 text-right">Gesamtgewinn</th><th class="px-3 py-2.5 text-right">Ø Rendite</th><th class="px-4 py-2.5 text-right">Max. Drawdown</th></tr></thead><tbody class="divide-y divide-[var(--ak-border)]">
