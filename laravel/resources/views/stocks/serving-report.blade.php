...
.chart{background:#f8fafc}.chart img{height:224px}
</style></head><body>
@php
...
<div class="section"><div class="section-title">{{ $t('Kerzenchart, Prognosen und Panel','Candlestick chart, forecasts and panel') }}</div><div class="chart"><img src="{{ $chartData }}" alt="{{ $t('Kerzenchart mit Prognosepfad, Wahrscheinlichkeiten und Panel-Sekundärachse','Candlestick chart with forecast path, probabilities and secondary panel axis') }}"></div></div>
...
@endphp
<div class="section"><div class="section-title">{{ $t('Externe KI-Bewertung','External AI review') }}</div>
<table class="data"><tbody>
<tr><td>{{ $t('Bewertung','Assessment') }}</td><td class="right"><strong class="{{ $reviewTone }}">{{ $reviewLabel }}</strong></td><td>{{ $t('Status','Status') }}</td><td class="right"><strong>{{ $reviewStatus }}</strong></td></tr>
<tr><td>{{ $t('Konfidenz','Confidence') }}</td><td class="right"><strong>{{ is_numeric($externalBuyReview->confidence)?$num($externalBuyReview->confidence,0).' %':'—' }}</strong></td><td>{{ $t('Modell / geprüft am','Model / reviewed at') }}</td><td class="right"><strong>{{ $externalBuyReview->model ?: '—' }}@if($externalBuyReview->researched_at) · {{ \Illuminate\Support\Carbon::parse($externalBuyReview->researched_at)->format($en?'Y-m-d H:i':'d.m.Y H:i') }}@endif</strong></td></tr>
</tbody></table>
<div class="review-comment"><strong>{{ $t('Externer Kommentar','External comment') }}</strong>{{ $reviewComment }}</div>
@if(count($externalBuyReviewPositiveFactors)||count($externalBuyReviewRiskFactors))<table class="columns"><tr><td class="card"><strong class="positive">{{ $t('Positive Faktoren','Positive factors') }}</strong><ul class="list">@forelse($externalBuyReviewPositiveFactors as $item)<li>{{ is_scalar($item)?$item:json_encode($item,JSON_UNESCAPED_UNICODE) }}</li>@empty<li>—</li>@endforelse</ul></td><td class="card"><strong class="negative">{{ $t('Risikofaktoren','Risk factors') }}</strong><ul class="list">@forelse($externalBuyReviewRiskFactors as $item)<li>{{ is_scalar($item)?$item:json_encode($item,JSON_UNESCAPED_UNICODE) }}</li>@empty<li>—</li>@endforelse</ul></td></tr></table>@endif
</div>
@endif
