<x-app-layout>
    ...
    <div class="flex h-[calc(100dvh-89px)] min-h-0 flex-col py-4 text-[var(--ak-text)]">
        ...
        <div class="min-h-0 flex-1 overflow-auto rounded-2xl border border-[var(--ak-border)] bg-[var(--ak-card)] shadow-[var(--ak-shadow)]">
            <table class="w-full min-w-[900px] table-fixed border-separate border-spacing-x-2 border-spacing-y-1.5 px-2 pb-2 text-left">
                <colgroup>
                    <col class="w-48">
                    @foreach ($rows as $row)
                        <col>
                ...
                <thead class="sticky top-0 z-20 bg-[#111020]/96 backdrop-blur-xl">
                    <tr>
                        ...
                        @foreach ($rows as $row)
                            <th class="rounded-2xl border border-violet-400/25 bg-[linear-gradient(145deg,rgba(139,92,246,.18),rgba(255,255,255,.055),var(--ak-surface-muted))] px-4 py-4 shadow-lg shadow-black/15">
                                ...
                                <td class="rounded-xl border px-4 py-3 text-center text-sm font-bold shadow-sm shadow-black/10 transition
                                    ...
                                    @elseif ($type === 'sector')
                                        <span class="inline-flex items-center justify-center gap-2">
                                            <x-sector-icon :sector="$row->sector" class="h-4 w-4 shrink-0 text-teal-500" />
                                            {{ $displayValue }}
                                        </span>
                                    @else
