<x-app-layout>
    ...
    <main id="model-overview-page" class="ak-body min-h-[calc(100dvh-73px)] py-5 sm:py-8">
        <div class="ak-container" x-data="{ horizon: {{ (int) $initialHorizon }}, variant: {{ \Illuminate\Support\Js::from($initialVariant ?? null) }} }">
            ...
            <header class="ak-card overflow-hidden border-cyan-400/30 p-5 sm:p-6">
                <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
                    ...
                    <div class="flex flex-wrap gap-2 xl:max-w-md xl:justify-end">
                        ...
                        @if($currentSignal?->has_quality_gate_buy)<span class="rounded-lg border border-amber-500 bg-amber-400 px-3 py-2 text-xs font-black text-slate-950 shadow-[0_8px_20px_rgba(245,158,11,.28)]">Quality Gate</span>@endif
                    </div>
                </div>
            ...
            @if($noLocalCoverage ?? false)
                <section class="ak-card mt-4 overflow-hidden border-rose-400/30 bg-rose-400/[.05] p-4 sm:p-5">
                    <p class="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[.16em] text-rose-400"><x-heroicon-o-exclamation-triangle class="h-3.5 w-3.5" />{{ __('Keine automatisierbare Konfiguration für diese Aktie') }}</p>
                    <p class="mt-2 text-[9px] leading-4 text-[var(--ak-muted)]">{{ __('Keine Horizont-/Modellvariante aus der Serving-Datenbank hat aktuell eine passende lokale Prognose, die AutomatedPortfolioService tatsächlich ausführen könnte (falscher Horizont, kein lokales Modell für :symbol, oder abweichende Modellvariante). Es gibt hier daher nichts, was sich sinnvoll zu einer Strategie hinzufügen ließe.', ['symbol' => $instrument->symbol]) }}</p>
                </section>
            @else
            <section class="ak-card mt-4 overflow-hidden border-violet-400/25">
                <div class="border-b border-[var(--ak-border)] p-4 sm:p-5">
                    ...
                    <p class="mt-1 text-[8px] leading-4 text-[var(--ak-muted)]">{{ __('Nur Horizont-/Modellvarianten, die die lokale, automatisch ausgeführte Prognose-Pipeline tatsächlich bedienen kann, werden hier angezeigt.') }}</p>
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach($horizons as $horizon)
                            <button type="button" @click="horizon = {{ $horizon['days'] }}; variant = null" class="rounded-lg border px-4 py-2 text-xs font-black transition" :class="horizon === {{ $horizon['days'] }} ? 'border-cyan-400/60 bg-cyan-400/[.12] text-cyan-400' : 'border-[var(--ak-border)] text-[var(--ak-muted)] hover:border-cyan-400/30'">
                                {{ $horizon['days'] }}T
                ...
                @foreach($horizons as $horizon)
                    <div x-cloak x-show="horizon === {{ $horizon['days'] }}" x-transition.opacity class="p-4 sm:p-5">
                        <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div class="flex flex-wrap gap-2"><button x-cloak x-show="variant !== null" type="button" @click="variant = null" class="rounded-md border border-slate-400/35 px-2 py-1 text-[9px] font-black text-slate-600">{{ __('Alle Modelle') }}</button><span class="rounded-md border border-violet-400/30 px-2 py-1 text-[9px] font-black text-violet-400">{{ __('Vergleichssieger') }}: {{ $horizon['preferred_variant'] === 'pure_tcn' ? 'Pure TCN' : 'Standard' }}</span><span class="rounded-md border px-2 py-1 text-[9px] font-black {{ $horizon['active_prediction_enabled'] ? 'border-emerald-400/35 text-emerald-400' : 'border-rose-400/35 text-rose-400' }}">{{ $horizon['active_prediction_enabled'] ? __('Prediction aktiv') : __('Prediction gesperrt') }}</span></div>
                        </div>
                        ...
                        <div class="grid gap-4 xl:grid-cols-2">
                            @foreach($horizon['variants'] as $variant)
                                ...
                                @endphp
                                <article x-cloak x-show="variant === null || variant === '{{ $variant['key'] }}'" x-transition.opacity class="model-variant-card {{ $variant['selected'] ? 'is-selected' : 'is-challenger' }} rounded-2xl border {{ $variantBorder }} bg-cyan-400/[.018] p-4">
                                    ...
                                    <dl class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
                                        @foreach([
                                            ...
                                            <div class="model-metric rounded-lg border border-[var(--ak-border)] p-2"><dt class="text-[7px] font-black uppercase tracking-wide text-[var(--ak-muted)]">{{ $metricLabel }}</dt><dd class="mt-1 text-xs font-black tabular-nums {{ $metricTone }}">{{ $metricValue }}</dd></div>
                                        @endforeach
                                    </dl>

                                    <div class="model-entry-policy mt-4 rounded-xl border border-[var(--ak-border)] p-3">
                                    ...
                                    {{-- Every variant reaching this point already passed
                                         LocalModelFeasibilityService in the controller -
                                         infeasible ones (wrong horizon, no local model, no
                                         local TCN variant) are filtered out before the view
                                         ever renders, not just warned about. --}}
