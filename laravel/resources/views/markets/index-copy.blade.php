    ...
    <div id="markets-page">
                    ...
                    <div class="space-y-5">
                        @foreach ($continentLabels as $continent => $continentLabel)
                            ...
                            <section class="min-w-0">
                                <div class="mb-2.5 flex h-10 items-center justify-between border-b border-orange-400/15 px-1 pb-2">
                                    <div class="flex items-center gap-2">
                                        <span class="grid h-8 w-8 place-items-center rounded-lg border border-orange-400/20 bg-orange-400/[.08] text-orange-400"><x-continent-icon :continent="$continent" class="h-5 w-5" /></span>
                                        <div>
                                ...
                                <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                                    @foreach ($continentMarkets as $exchange)
                                        @php
                                            ...
                                            $scoreBadgeHue = $score !== null
                                                ? round(min(120, max(0, (($score - 3) / 4) * 120)))
                                                : 0;
                                            $scoreBadgeStyle = $score !== null
                                                ? "background-color:hsl({$scoreBadgeHue} 68% 48%);border-color:hsl({$scoreBadgeHue} 72% 66%)"
                                                : 'background-color:rgba(100,116,139,.45);border-color:rgba(148,163,184,.45)';
                                            $target = route('stocks.index', ['exchange' => $exchange->code]);
                                        @endphp
                                        <x-dashboard.card onclick="window.location.href=@js($target)" class="ak-card-static ak-dashboard-card ak-cyan-dashboard-card ak-market-card-prominent min-h-0 cursor-pointer overflow-hidden p-0">
                                            <div class="ak-market-card-head flex items-center justify-between gap-2 border-b border-orange-400/30 p-2.5">
                                                <div class="flex min-w-0 items-center gap-2">
                                                    <span class="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-orange-400/25 bg-orange-400/10 text-base">{{ $countryFlag($exchange->country) }}</span>
                                                    <div class="min-w-0">
                                                        <h3 class="truncate text-[15px] font-black text-[var(--ak-text)]">{{ $exchange->reference_name ?: $exchange->code }}</h3>
                                                        <p class="truncate text-[9px] font-bold text-[var(--ak-muted)]">{{ $exchange->code }} · {{ $exchange->name }}</p>
                                                    </div>
                                                </div>
                                                <span class="shrink-0 rounded-md border border-orange-400/25 bg-orange-400/10 px-1.5 py-1 text-[8px] font-black text-orange-400">{{ $exchange->instrument_count }} {{ __('Aktien') }}</span>
                                            </div>

                                            <div class="mx-2.5 mb-2.5 mt-2 grid grid-cols-3 gap-1 text-center">
                                                <span class="min-w-0">
                                                    <small class="block truncate text-[8px] font-black uppercase text-[var(--ak-muted)]">{{ __('Indexstand') }}</small>
                                                    <b data-market-price="{{ $exchange->code }}" class="mt-0.5 block truncate text-sm font-black tabular-nums text-[var(--ak-text)]"><span>{{ is_numeric($exchange->market_price) ? number_format($exchange->market_price, 2, ',', '.') : '—' }}</span> <small class="text-[7px] text-[var(--ak-muted)]">{{ $currencySymbol($exchange->market_currency) }}</small></b>
                                                </span>
                                                <span class="min-w-0 border-x border-orange-400/25">
                                                    <small class="block truncate text-[8px] font-black uppercase text-[var(--ak-muted)]">{{ __('Performance 1T') }}</small>
                                                    <b data-market-change="{{ $exchange->code }}" class="mt-0.5 block truncate text-sm font-black tabular-nums {{ ($exchange->market_change ?? 0) >= 0 ? 'text-emerald-300' : 'text-rose-300' }}">{{ is_numeric($exchange->market_change) ? (($exchange->market_change > 0 ? '+' : '').number_format($exchange->market_change, 2, ',', '.').' %') : '—' }}</b>
                                                </span>
                                                <span class="min-w-0">
                                                    <small class="block truncate text-[8px] font-black uppercase text-[var(--ak-muted)]">{{ __('KI-Score') }}</small>
                                                    <b class="mx-auto mt-0.5 inline-flex h-5 min-w-[52px] items-center justify-center rounded-md border px-2 text-xs font-black tabular-nums text-slate-950" style="{{ $scoreBadgeStyle }}">{{ $score !== null ? number_format($score, 1, ',', '.') : '—' }} <small class="ml-0.5 text-[7px] opacity-65">/10</small></b>
                                                </span>
                                            </div>
        ...
        <style>
            ...
            .ak-market-dashboard-card {
                ...
                box-shadow: 0 12px 30px rgba(2, 132, 199, .10), inset 0 1px 0 rgba(251,146,60, .035) !important;
            ...
            .ak-market-tab-active {
                border-color: color-mix(in srgb, rgb(34 211 238) 42%, var(--ak-border));
