...
<style>
    ...
    .cc-banner{position:fixed;z-index:9998;left:50%;bottom:18px;width:min(960px,calc(100% - 28px));transform:translateX(-50%);padding:20px 54px 20px 20px;border:1px solid var(--cc-line);border-radius:20px;background:linear-gradient(135deg,rgba(13,42,56,.99),rgba(5,21,34,.99));box-shadow:0 28px 90px rgba(0,0,0,.58),inset 0 1px rgba(255,255,255,.04);color:var(--cc-text)}
    .cc-banner-close{position:absolute;right:12px;top:12px;width:32px;height:32px;border:1px solid var(--cc-line);border-radius:9px;background:transparent;color:#cde6e9;font-size:19px;line-height:1;cursor:pointer}.cc-banner-close:hover{border-color:rgba(86,231,241,.52);background:rgba(86,231,241,.08)}
    ...
    [data-cc-hidden]{display:none!important}@media(max-width:720px){.cc-banner{bottom:10px;padding:16px 48px 16px 16px}.cc-grid{grid-template-columns:1fr;gap:14px}.cc-actions{justify-content:stretch}.cc-btn{flex:1}.cc-modal{padding:18px}.cc-modal-actions .cc-btn{flex:1}}
</style>
<div data-cookie-consent data-cc-hidden>
    <div class="cc-backdrop" data-cc-backdrop data-cc-hidden></div>
    <section class="cc-banner" data-cc-banner role="dialog" aria-labelledby="cc-title">
        <button class="cc-banner-close" type="button" data-cc-dismiss aria-label="{{ __('Schließen und nur notwendige Cookies verwenden') }}">×</button>
    ...
    <section class="cc-modal" data-cc-modal data-cc-hidden role="dialog" aria-modal="true" aria-labelledby="cc-settings-title">
        ...
        <div class="cc-modal-actions"><button class="cc-btn" type="button" data-cc-back>{{ __('Zurück') }}</button><button class="cc-btn" type="button" data-cc-necessary>{{ __('Nur notwendige') }}</button><button class="cc-btn cc-primary" type="button" data-cc-save>{{ __('Auswahl speichern') }}</button></div>
    </section>
</div>
<button class="cc-settings-trigger" type="button" data-cc-open>{{ __('Cookie-Einstellungen') }}</button>
<script>
(() => {
    const key='aktienki-cookie-consent-v1',version=1,serverAccepted=@js((bool) (auth()->user()?->accepted_cookie_notice ?? false)),root=document.querySelector('[data-cookie-consent]');
    if(!root)return;
    ...
    const necessaryValue=()=>({version,necessary:true,preferences:false,analytics:false,marketing:false,savedAt:null});
    const read=()=>{try{const value=JSON.parse(localStorage.getItem(key)||'null');if(value?.version===version)return value}catch(_){}return serverAccepted||document.cookie.split(';').some(cookie=>cookie.trim()==='aki_cookie_consent=1')?necessaryValue():null};
    ...
    const close=()=>{root.setAttribute('data-cc-hidden','');backdrop.setAttribute('data-cc-hidden','');banner.removeAttribute('data-cc-hidden');modal.setAttribute('data-cc-hidden','')};
    const openBanner=()=>{root.removeAttribute('data-cc-hidden');backdrop.setAttribute('data-cc-hidden','');banner.removeAttribute('data-cc-hidden');modal.setAttribute('data-cc-hidden','')};
    const openSettings=event=>{const current=read()||{},source=event?.currentTarget;returnFocus=source instanceof HTMLElement?source:document.activeElement;root.removeAttribute('data-cc-hidden');backdrop.removeAttribute('data-cc-hidden');banner.setAttribute('data-cc-hidden','');modal.removeAttribute('data-cc-hidden');root.querySelectorAll('[data-cc-category]').forEach(input=>input.checked=Boolean(current[input.dataset.ccCategory]));modal.querySelector('button')?.focus()};
    const restoreFocus=()=>{const target=returnFocus;returnFocus=null;requestAnimationFrame(()=>target?.focus?.())};
    const returnFromSettings=()=>{read()?close():openBanner();restoreFocus()};
    const save=choices=>{const value={version,necessary:true,preferences:Boolean(choices.preferences),analytics:Boolean(choices.analytics),marketing:Boolean(choices.marketing),savedAt:new Date().toISOString()};try{localStorage.setItem(key,JSON.stringify(value))}catch(_){}try{document.cookie='aki_cookie_consent=1; Max-Age=15552000; Path=/; SameSite=Lax'+(location.protocol==='https:'?'; Secure':'')}catch(_){}apply(value);close()};
    ...
    document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!modal.hasAttribute('data-cc-hidden')){event.preventDefault();dismiss()}});
