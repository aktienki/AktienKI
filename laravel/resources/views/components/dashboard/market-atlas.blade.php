...
<section id="market-atlas" class="ak-dashboard-atlas ak-dashboard-panel flex h-full flex-col scroll-mt-24 rounded-[1.5rem] border border-white/10 bg-white/[.025] p-4 shadow-xl shadow-black/10 sm:p-5">
    <div class="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div>
            <p class="text-xs font-black uppercase tracking-[.18em] text-violet-300">{{ __('Globaler KI-Atlas') }}</p>
            <p class="mt-1 text-xs text-slate-400">{{ __('Durchschnittlicher KI-Score der analysierten Aktien je Land') }}</p>
        </div>
    ...
    <div
        ...
        <svg
            ...
            role="img"
            aria-label="{{ __('Weltkarte der durchschnittlichen KI-Scores nach Ländern') }}">
        </svg>
    ...
    <div class="mt-2 w-full">
        <div class="ak-atlas-scale grid w-full grid-cols-10 gap-px opacity-50" aria-label="{{ __('KI-Score-Farbskala von 0 bis 10') }}">
            @for ($step = 0; $step < 10; $step++)
                <i class="ak-atlas-scale-step h-1" style="--ak-atlas-step: {{ $step }}; background-color: hsl({{ ($step * 120) / 9 }}, 62%, 50%)"></i>
            @endfor
        </div>
        <div class="mt-1 flex justify-between text-[8px] font-medium text-slate-600">
            <span>0 · {{ __('Schwach') }}</span>
            <span>5</span>
            <span>{{ __('Stark') }} · 10</span>
        </div>
    </div>
</section>
