                ...
                @foreach($sections as $section)
                    <section class="concept-card concept-main-card" x-show="active === '{{ $section['id'] }}'" x-cloak>
                        ...
                        @if($section['kind'] === 'today-focus')
                            ...
                            @if(count($section['opportunities'] ?? []))
                                <div class="mt-5">
                                    <h3 class="mb-3 flex items-center gap-2 text-sm font-black text-[var(--ak-text)]">
                                        <x-heroicon-o-bolt class="h-4 w-4 text-amber-500" />
                                        {{ __('Trading Opportunities') }}
                                    </h3>
                                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 items-stretch">
                                        @foreach($section['opportunities'] as $highlight)
                                            @include('dashboard-concept._highlight-card', ['highlight' => $highlight])
                                        @endforeach
                                    </div>
                                </div>
                                    ...
                                    <table class="w-full min-w-[900px] border-collapse text-[11px]">
                                        <thead>
                                            <tr class="text-[9px] font-black uppercase tracking-wide text-[var(--ak-muted)]">
                                                ...
                                                <th class="border-0 pb-1.5 px-2 text-left font-black">{{ __('Variante') }}</th>
                                                ...
                                                <th class="border-0 pb-1.5 px-2 text-right font-black">{{ __('Erwartete Rendite') }}</th>
                                                <th class="border-0 pb-1.5 pl-2 text-center font-black">{{ __('Quality Gate') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @php
                                            ...
                                            @foreach($section['rows'] as $row)
                                                <tr class="border-t border-[var(--ak-border)]">
                                                    ...
                                                    <td class="px-2 py-1.5 text-[var(--ak-muted)]">{{ $row['variant'] }}</td>
                                                    ...
                                                    <td class="px-2 py-1.5 text-right tabular-nums text-[var(--ak-text)]">{{ $row['risk'] === null ? '—' : $row['risk'].'/5' }}</td>
                                                    <td class="px-2 py-1.5 text-right tabular-nums {{ ($row['expected_return'] ?? 0) >= 0 ? 'text-emerald-500' : 'text-rose-500' }}">{{ $row['expected_return'] === null ? '—' : number_format($row['expected_return'], 1, ',', '.').' %' }}</td>
                                                    <td class="py-1.5 pl-2 text-center font-black {{ $row['quality_gate_passed'] === true ? 'text-emerald-500' : ($row['quality_gate_passed'] === false ? 'text-rose-500' : 'text-[var(--ak-muted)]') }}">{{ $row['quality_gate_passed'] === true ? __('Bestanden') : ($row['quality_gate_passed'] === false ? __('Nicht bestanden') : '—') }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
