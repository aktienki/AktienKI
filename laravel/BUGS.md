...
32. ✅ **Tatsächliche Ursache von Punkt 28: `php8.5-fpm.service` hatte `UMask=0022` (systemd-Unit-Default).** Jede von PHP-FPM live neu erzeugte Datei (z.B. ein Blade-Compile für eine Komponente, die `view:cache` beim Deploy nicht statisch findet) bekam dadurch Modus `644`/`755` statt `664`/`775` — Gruppe (`www-data`) ohne Schreibrecht auf ihre eigenen frisch erzeugten Dateien, unabhängig von jeder Verzeichnis-ACL. Das erklärt, warum der ACL-Fix aus Punkt 28 nur für die zum Fix-Zeitpunkt existierenden Dateien half und für später live erzeugte erneut fehlschlug. Dauerhaft behoben per systemd-Drop-in `/etc/systemd/system/php8.5-fpm.service.d/umask.conf` (`UMask=0002`) + `daemon-reload` + Neustart, verifiziert über `/proc/<worker-pid>/status` (jetzt `Umask: 0002` für Master und alle Worker) und einen echten End-zu-Ende-Test (`view:clear` + Live-Request → neu erzeugte Datei zeigt `-rwxrwxr-x`, `www-data` kann sie beschreiben).
