...
28. 🟡 **Produktions-"Server Error" nach Deploy — ACL-Rechteproblem, nicht der Code.** `storage/framework/views` hatte einen zu engen ACL-`mask` (effektiv `r-x` statt `rwx`) auf einzelnen kompilierten Blade-Dateien, sodass `www-data` (PHP-FPM-Nutzer) beim Live-Neukompilieren eines noch nicht vor-cachten Views per `touch()` scheiterte ("Utime failed: Operation not permitted"). Betraf nur Views, die `view:cache` beim Deploy nicht statisch findet (z.B. dynamisch eingebundene Komponenten). Erst per `setfacl -R` (Access- **und** Default-ACL) behoben — dieser Fix hielt aber NICHT: der Fehler trat Stunden später erneut auf, für brandneue, von `www-data` selbst live erzeugte Dateien, trotz korrekter Default-ACL auf dem Verzeichnis. Echte Ursache siehe Punkt 32.
    *Update:* Siehe Punkt 32 für den tatsächlichen, dauerhaften Fix.
29. ✅ **Screener-Filterleiste: "Nur Quality-Gate bestanden"-Checkbox überlappte andere Filterfelder — drei echte, verschachtelte CSS-Bugs statt eines Breiten-Problems.** Trotz mehrfacher, jeweils serverseitig verifizierter "Fixes" blieb der visuelle Fehler bestehen, weil die eigentliche Ursache eine Layout-Ebene tiefer lag, die reines HTML/CSS-Quelltext-Grep nicht zeigt: (1) Ab `min-width:1024px` schaltet `.screener-filter-bar` auf ein explizites 12-Spalten-CSS-Grid mit Name-basierten `grid-column`/`order`-Regeln pro Feld; die Annahme, `label:first-child` treffe die Quality-Gate-Checkbox, war falsch — das eigentliche erste Kind ist die Such-Box, die Checkbox hatte gar keine eigene Regel und kollidierte per Default-Platzierung. (2) Eine hartkodierte `width:280px` überlief bei schmaleren Desktop-Breiten die eigene `grid-column:span`-Spur. (3) Die generische `input,select,button{height:2.35rem!important}`-Regel (Desktop **und** die separate `<768px`-Mobile-Variante) traf versehentlich auch die Checkbox selbst und streckte sie zu einem schmalen hohen Balken. Erst durch echtes Headless-Browser-Rendering (Playwright, gegen Produktion, mit Bounding-Box-Messung über mehrere Breakpoints) statt Server-Text-Inspektion gefunden und verifiziert.
    *Dateien:* `resources/css/app.css`, `resources/views/screener/index.blade.php`
    *Lehre:* Bei "sieht für mich richtig aus, aber Nutzer sieht es kaputt" nicht weiter am Quelltext grep'en — echten Browser (Playwright) gegen die echte URL rendern und Bounding-Boxes messen.
30. ✅ **KI-Bewertung (Aktien-Detailseite): Quality Gate/Externe Bewertung als echte Status-Donuts statt Gradient-Gauge, Konfidenz raus, Ø Profit/Trade rein.** Quality Gate ist jetzt ein 2-Segment-Donut (rot=nein/grün=ja, neuer Component-Type `binary`) statt der 5-stufigen "wie gut"-Gradientenskala, die für ein reines Pass/Fail nie gepasst hat. Neuer "Externe Bewertung"-Donut (Component-Type `quad`, 4 exklusive Segmente statt kumulativ) zeigt alle vier echten Verdict-Zustände (`NO_OBJECTION`/`CAUTION`/`OBJECTION`/`INSUFFICIENT_EVIDENCE`) statt sie auf ja/nein zu verflachen. Dabei entdeckt: der externe-Buy-Review-Abruf war in **beiden** Fundstellen (`StockController::show()`, `ServingStockLegacyViewService::data()`) hart an `PlanLevel::Pro` gekoppelt — der neue Donut hätte für Free/Plus-Nutzer permanent leer/grau angezeigt, während alle anderen Donuts in derselben Reihe echte Werte zeigen. Auf Wunsch entkoppelt, Daten sind jetzt tarifunabhängig sichtbar.
    *Dateien:* `resources/views/components/segmented-score-donut.blade.php`, `resources/views/stocks/show.blade.php`, `app/Http/Controllers/StockController.php`, `app/Services/ServingStockLegacyViewService.php`

32. 🟡 **Teilursache von Punkt 28: `php8.5-fpm.service` hatte `UMask=0022` (systemd-Unit-Default).** Jede von PHP-FPM live neu erzeugte Datei bekam dadurch Modus `644`/`755` statt `664`/`775` — Gruppe (`www-data`) ohne Schreibrecht auf ihre eigenen frisch erzeugten Dateien, unabhängig von jeder Verzeichnis-ACL. Behoben per systemd-Drop-in `/etc/systemd/system/php8.5-fpm.service.d/umask.conf` (`UMask=0002`). Das war aber nur die HÄLFTE der Ursache — der Fehler trat am 15.09. erneut auf, dieses Mal für eine Datei, die beim Deploy selbst (`view:cache`, ausgeführt als `aktienki` via `sudo -u`) frisch erzeugt wurde. `aktienki`s eigene Sudo-Shell hat ihr EIGENES Umask (0022), komplett unabhängig vom php-fpm-Systemd-Fix. Siehe Punkt 34 für die tatsächlich vollständige Behebung.
    *Update:* Siehe Punkt 34.

33. ✅ **Screener-Ranking konnte ein Aktie mit fehlgeschlagenem Quality Gate auf Platz 1 setzen — der Composite-Score benutzte das falsche Modell.** Konkreter Fall: IHI Corporation (IWJ.DE), Composite-Score 94 (nahezu perfekt, Rang 1), obwohl die Detailseite überall schlechte Werte zeigte. Ursache: `ServingScreenerService::stock()` wählte das "primäre" Modell für die Score-Berechnung hart nach `20T → 40T → 10T`-Präferenz, **unabhängig davon, welches Horizont+Variante das BUY-Signal tatsächlich ausgelöst hat** (`buy_scopes`). Bei IWJ.DE kommt das BUY-Signal von 40T/Standard (Quality Gate **fehlgeschlagen**, Profit-Faktor 1.57), aber der hartkodierte 20T-Default landete bei 20T/Pure-TCN (Quality Gate **bestanden**) — derselbe horizontfremde Fehler, der schon Punkt (Historische-Signale-Chart, heute früher behoben) betraf, hier aber mit Auswirkung auf die eigentliche Rangliste. Die Aktien-Detailseite selbst war schon korrekt (nutzt `resolveSignalScope()`), nur der Screener/die Rangliste nicht.
    *Fix:* Die Auswahllogik (`resolveSignalScope()`) aus `ServingStockLegacyViewService` in eine gemeinsame Klasse `App\Services\SignalScopeResolver` ausgelagert und in **beide** Stellen (`ServingStockLegacyViewService`, `ServingScreenerService::stock()`) eingebaut, damit Detailseite und Ranking nie wieder unabhängig voneinander unterschiedliche Modelle für dasselbe Signal auswählen können. IWJ.DE fällt dadurch jetzt korrekt von 94 auf 40. Stichprobe über 683 Aktien: keine Nullwerte, Score-Verteilung 9-94 (Ø 40,5), unauffällig.
    *Dateien:* `app/Services/SignalScopeResolver.php` (neu), `app/Services/ServingStockLegacyViewService.php`, `app/Services/ServingScreenerService.php`

34. ✅ **Tatsächlich vollständige Ursache/Behebung von Punkt 28/32: `deploy.sh`s `as_app()`-Wrapper hatte selbst kein `umask 002`.** Zwei GETRENNTE Prozesse erzeugen Dateien in `storage/framework/views`: PHP-FPM (live, bei Bedarf) — per Punkt 32 behoben — und `deploy.sh`s `view:cache`/`config:cache` etc., ausgeführt als `sudo -u aktienki` bei jedem Deploy. Letzterer lief mit `aktienki`s normalem Sudo-Shell-Umask (0022), komplett unabhängig vom php-fpm-Systemd-Fix — jeder Deploy konnte also weiterhin frische Dateien mit `mask::r-x` erzeugen, sobald `view:cache` eine Komponente kompiliert, die vorher noch nicht im Cache lag (z.B. nach Hinzufügen einer neuen Seite). Behoben in `as_app()` selbst: `sudo -u "$APP_USER" env -C "$1" bash -c 'umask 002; exec "$@"' bash "${@:2}"` statt des direkten `env -C ... "${@:2}"` — sowohl in der Repo-Kopie (`infrastructure/deploy.sh`) als auch der laufenden Kopie (`/home/aktienki/deploy.sh`, mit Backup). Verifiziert über einen echten Deploy-Lauf: alle frisch erzeugten Dateien zeigen jetzt `mask::rwx`, `www-data` kann sie direkt beschreiben (getestet per `touch`).
    *Lehre:* Bei einem wiederkehrenden Rechte-Fehler nicht nach dem ERSTEN plausiblen Übeltäter aufhören zu suchen — hier gab es zwei unabhängige Prozesse (Live-Traffic vs. Deploy-Skript), die dieselbe Fehlermeldung über verschiedene Wege auslösen konnten, und der erste Fix (Punkt 32) hat nur einen davon getroffen.

## 🔴 Offen — Twelve-Data-Fundamentaldaten (Nachladung)

31. **`fundamentals:import-twelve-data --missing-only` — nur 82 von 311 fehlenden Aktien erfolgreich nachgeladen (~26%).** Gleiches Muster wie Punkt 25 (Kurshistorie): viele scheiternde Symbole sind kryptische `.DE`-Ticker (`9MD.DE`, `YZCA.DE`, `ZNHH.DE`, `BIG.DE` …), die Twelve Data als "symbol or figi parameter is missing or invalid" zurückweist — vermutlich dieselbe fehlerhafte Symbol-Auflösung für nicht-originär-deutsche Notierungen. Nicht weiter untersucht (Twelve-Data-Rate-Limit zusätzlich verschärfend: mehrfach "API credits for the current minute" trotz eingebauter Wiederholung).
