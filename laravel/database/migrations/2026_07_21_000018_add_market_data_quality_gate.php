...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('predictions', function (Blueprint $table): void {
            $table->string('market_data_quality_status', 24)->default('unknown');
            $table->decimal('market_data_age_hours', 12, 4)->nullable();
            $table->decimal('market_data_coverage', 8, 6)->nullable();
            $table->boolean('market_data_quality_veto_used')->default(false);
            $table->jsonb('market_data_quality_details')->default(DB::raw("'{}'::jsonb"));
            $table->index(
                ['market_data_quality_status', 'market_data_quality_veto_used'],
                'predictions_market_data_quality_idx'
            );
        });
    ...
    public function down(): void
    {
        ...
        Schema::table('predictions', function (Blueprint $table): void {
            $table->dropIndex('predictions_market_data_quality_idx');
            $table->dropColumn([
                'market_data_quality_status', 'market_data_age_hours',
                'market_data_coverage', 'market_data_quality_veto_used',
                'market_data_quality_details',
            ]);
        });
        $this->createPublicView(false);
    }

    private function createPublicView(bool $withDataQuality): void
    {
        DB::statement('DROP VIEW IF EXISTS public_prediction_models');
        $extra = $withDataQuality
            ? ', p.market_data_quality_status, p.market_data_age_hours, p.market_data_coverage, p.market_data_quality_veto_used, p.market_data_quality_details'
            : '';
        DB::statement(<<<SQL
