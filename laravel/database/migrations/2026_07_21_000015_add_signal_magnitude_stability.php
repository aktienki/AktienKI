...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('predictions', function (Blueprint $table): void {
            $table->boolean('signal_magnitude_stable')->nullable();
            $table->decimal('signal_magnitude_deviation', 10, 6)->nullable();
            $table->index(
                ['signal_magnitude_stable', 'signal_consistency_veto_used'],
                'predictions_signal_magnitude_idx'
            );
        });
        DB::statement(<<<'SQL'
            UPDATE predictions SET
                signal_magnitude_stable = NULLIF(signal_consistency_details->>'magnitude_stable', '')::boolean,
                signal_magnitude_deviation = NULLIF(signal_consistency_details->>'magnitude_relative_deviation', '')::numeric
            SQL);
    ...
    public function down(): void
    {
        ...
        Schema::table('predictions', function (Blueprint $table): void {
            $table->dropIndex('predictions_signal_magnitude_idx');
            $table->dropColumn([
                'signal_magnitude_stable', 'signal_magnitude_deviation',
            ]);
        });
        $this->createPublicView(false);
    }

    private function createPublicView(bool $withMagnitude): void
    {
        DB::statement('DROP VIEW IF EXISTS public_prediction_models');
        $extra = $withMagnitude
