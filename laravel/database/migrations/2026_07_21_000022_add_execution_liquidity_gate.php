...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('predictions', function (Blueprint $table): void {
            $table->string('execution_liquidity_status', 24)->default('unknown');
            $table->decimal('median_dollar_volume', 20, 2)->nullable();
            $table->decimal('volume_coverage', 8, 6)->nullable();
            $table->boolean('execution_liquidity_veto_used')->default(false);
            $table->jsonb('execution_liquidity_details')->default(DB::raw("'{}'::jsonb"));
            $table->index(
                ['execution_liquidity_status', 'execution_liquidity_veto_used'],
                'predictions_execution_liquidity_idx'
            );
        });
    ...
    public function down(): void
    {
        ...
        Schema::table('predictions', function (Blueprint $table): void {
            $table->dropIndex('predictions_execution_liquidity_idx');
            $table->dropColumn([
                'execution_liquidity_status', 'median_dollar_volume',
                'volume_coverage', 'execution_liquidity_veto_used',
                'execution_liquidity_details',
            ]);
        });
        $this->createPublicView(false);
    }

    private function createPublicView(bool $withLiquidity): void
    {
        DB::statement('DROP VIEW IF EXISTS public_prediction_models');
        $extra = $withLiquidity
            ? ', p.execution_liquidity_status, p.median_dollar_volume, p.volume_coverage, p.execution_liquidity_veto_used, p.execution_liquidity_details'
            : '';
        DB::statement(<<<SQL
            ...
            SELECT p.id, p.instrument_id, p.ai_type, p.prediction_time, p.timeframe,
                   ...
                   p.ensemble_dispersion_stable, p.ensemble_relative_dispersion,
                   p.ensemble_dispersion_veto_used,
                   p.market_data_quality_status, p.market_data_age_hours,
                   p.market_data_coverage, p.market_data_quality_veto_used,
                   p.market_data_quality_details,
                   p.economic_edge_return, p.economic_edge_required,
                   p.economic_edge_veto_used, p.economic_edge_details
                   {$extra}
            FROM predictions p
