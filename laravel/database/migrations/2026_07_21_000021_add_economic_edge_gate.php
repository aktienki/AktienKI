...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('predictions', function (Blueprint $table): void {
            $table->decimal('economic_edge_return', 12, 8)->nullable();
            $table->decimal('economic_edge_required', 12, 8)->nullable();
            $table->boolean('economic_edge_veto_used')->default(false);
            $table->jsonb('economic_edge_details')->default(DB::raw("'{}'::jsonb"));
            $table->index(
                ['economic_edge_veto_used', 'economic_edge_return'],
                'predictions_economic_edge_idx'
            );
        });
    ...
    public function down(): void
    {
        ...
        Schema::table('predictions', function (Blueprint $table): void {
            $table->dropIndex('predictions_economic_edge_idx');
            $table->dropColumn([
                'economic_edge_return', 'economic_edge_required',
                'economic_edge_veto_used', 'economic_edge_details',
            ]);
        });
        $this->createPublicView(false);
    }

    private function createPublicView(bool $withEdge): void
    {
        DB::statement('DROP VIEW IF EXISTS public_prediction_models');
        $extra = $withEdge
            ? ', p.economic_edge_return, p.economic_edge_required, p.economic_edge_veto_used, p.economic_edge_details'
            : '';
        DB::statement(<<<SQL
