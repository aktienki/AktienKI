...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('predictions', function (Blueprint $table): void {
            $table->decimal('prediction_plausibility_limit', 10, 6)->nullable();
            $table->boolean('prediction_outlier_detected')->default(false);
            $table->boolean('prediction_outlier_veto_used')->default(false);
            $table->jsonb('prediction_plausibility_details')->default(DB::raw("'{}'::jsonb"));
            $table->index(
                ['prediction_outlier_detected', 'prediction_outlier_veto_used'],
                'predictions_prediction_outlier_idx'
            );
        });

        DB::statement(<<<'SQL'
            UPDATE predictions SET
                prediction_plausibility_limit = NULLIF(metadata->'prediction_plausibility'->>'limit', '')::numeric,
                prediction_outlier_detected = COALESCE((metadata->'prediction_plausibility'->>'outlier_detected')::boolean, false),
                prediction_outlier_veto_used = COALESCE((metadata->'prediction_plausibility'->>'veto_used')::boolean, false),
                prediction_plausibility_details = COALESCE(metadata->'prediction_plausibility', '{}'::jsonb)
            SQL);

        $this->createPublicView();
    }
    ...
    public function down(): void
    {
        ...
        Schema::table('predictions', function (Blueprint $table): void {
            $table->dropIndex('predictions_prediction_outlier_idx');
            $table->dropColumn([
                'prediction_plausibility_limit', 'prediction_outlier_detected',
                'prediction_outlier_veto_used', 'prediction_plausibility_details',
            ]);
        });
        $this->createPreviousPublicView();
    }

    private function createPublicView(): void
    {
        DB::statement('DROP VIEW IF EXISTS public_prediction_models');
        DB::statement(<<<'SQL'
    ...
    private function createPreviousPublicView(): void
    {
        DB::statement(<<<'SQL'
            CREATE VIEW public_prediction_models AS
            SELECT p.id, p.instrument_id, p.ai_type, p.prediction_time, p.timeframe,
